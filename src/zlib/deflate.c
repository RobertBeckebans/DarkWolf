...
const char deflate_copyright[] = " deflate 1.3.2 Copyright 1995-2026 Jean-loup Gailly and Mark Adler ";
/*
...
int ZEXPORT deflatePrime( z_streamp strm, int bits, int value )
{
	...
	s = strm->state;
#ifdef LIT_MEM
	if( bits < 0 || bits > 16 || ( uchf* )s->d_buf < s->pending_out + ( ( Buf_size + 7 ) >> 3 ) ) {
		return Z_BUF_ERROR;
	}
#else
...
z_size_t ZEXPORT deflateBound_z( z_streamp strm, z_size_t sourceLen )
{
	...
	if( s->w_bits != 15 || s->hash_bits != 8 + 7 ) {
		bound = s->w_bits <= s->hash_bits && s->level ? fixedlen : storelen;
		return bound + wraplen < bound ? ( z_size_t )-1 : bound + wraplen;
	}

	/* default settings: return tight bound for that case -- ~0.03% overhead
	   plus a small constant */
	bound = sourceLen + ( sourceLen >> 12 ) + ( sourceLen >> 14 ) + ( sourceLen >> 25 ) + 13 - 6 + wraplen;
...
int ZEXPORT deflateCopy( z_streamp dest, z_streamp source )
{
	...
	zmemcpy( ds->prev, ss->prev, ( ss->slid || ss->strstart - ss->insert > ds->w_size ? ds->w_size : ss->strstart - ss->insert ) * sizeof( Pos ) );
...
local uInt longest_match( deflate_state* s, IPos cur_match )
{
	...
	do {
		 ...
		 */
		do {
		} while( *++scan == *++match && *++scan == *++match && *++scan == *++match && *++scan == *++match && *++scan == *++match && *++scan == *++match && *++scan == *++match && *++scan == *++match &&
				 scan < strend );
