...
static void			  Con_DestroyResources();
static void			  Con_CreateChildWindows( HWND hWnd );
static void			  Con_EnsureVisibleCaret();
...
static void Con_LayoutChildren( HWND hWnd )
{
	...
	if( s_wcd.hwndErrorBox ) {
		MoveWindow( s_wcd.hwndErrorBox, SYS_PAD, errorTop, clientW - ( SYS_PAD * 2 ), SYS_TOP_BANNER_H, TRUE );
	}

	if( s_wcd.hwndBuffer ) {
		MoveWindow( s_wcd.hwndBuffer, SYS_PAD, bufferTop, clientW - ( SYS_PAD * 2 ), bufferHeight, TRUE );
	}

	if( s_wcd.hwndInputLine ) {
		MoveWindow( s_wcd.hwndInputLine, SYS_PAD, inputY, clientW - ( SYS_PAD * 2 ), SYS_INPUT_H, TRUE );
	}

	if( s_wcd.hwndButtonCopy ) {
		MoveWindow( s_wcd.hwndButtonCopy, SYS_PAD, buttonsY, SYS_BUTTON_W, SYS_BUTTON_H, TRUE );
	}

	if( s_wcd.hwndButtonClear ) {
		MoveWindow( s_wcd.hwndButtonClear, SYS_PAD + SYS_BUTTON_W + SYS_BUTTON_GAP, buttonsY, SYS_BUTTON_W, SYS_BUTTON_H, TRUE );
	}

	if( s_wcd.hwndButtonQuit ) {
		MoveWindow( s_wcd.hwndButtonQuit, clientW - SYS_PAD - SYS_BUTTON_W, buttonsY, SYS_BUTTON_W, SYS_BUTTON_H, TRUE );
	}
}
...
static HFONT Con_CreateUIFont( HWND hWnd, int pointSize, const char* face, int weight, int fixedPitch )
{
	...
	hFont = CreateFontA(
		height, 0, 0, 0, weight, FALSE, FALSE, FALSE, DEFAULT_CHARSET, OUT_DEFAULT_PRECIS, CLIP_DEFAULT_PRECIS, CLEARTYPE_QUALITY, ( fixedPitch ? FIXED_PITCH : VARIABLE_PITCH ) | FF_DONTCARE, face );
...
static void Con_CreateChildWindows( HWND hWnd )
{
	s_wcd.hwndInputLine = CreateWindowExA( 0, "edit", "", WS_CHILD | WS_VISIBLE | WS_BORDER | ES_LEFT | ES_AUTOHSCROLL, 0, 0, 0, 0, hWnd, ( HMENU )( INT_PTR )INPUT_ID, g_wv.hInstance, NULL );

	s_wcd.hwndButtonCopy = CreateWindowExA( 0, "button", "Copy", WS_CHILD | WS_VISIBLE | BS_PUSHBUTTON, 0, 0, 0, 0, hWnd, ( HMENU )( INT_PTR )COPY_ID, g_wv.hInstance, NULL );

	s_wcd.hwndButtonClear = CreateWindowExA( 0, "button", "Clear", WS_CHILD | WS_VISIBLE | BS_PUSHBUTTON, 0, 0, 0, 0, hWnd, ( HMENU )( INT_PTR )CLEAR_ID, g_wv.hInstance, NULL );

	s_wcd.hwndButtonQuit = CreateWindowExA( 0, "button", "Quit", WS_CHILD | WS_VISIBLE | BS_PUSHBUTTON, 0, 0, 0, 0, hWnd, ( HMENU )( INT_PTR )QUIT_ID, g_wv.hInstance, NULL );

	s_wcd.hwndBuffer = CreateWindowExA(
		WS_EX_CLIENTEDGE, "edit", "", WS_CHILD | WS_VISIBLE | WS_VSCROLL | ES_LEFT | ES_MULTILINE | ES_AUTOVSCROLL | ES_READONLY, 0, 0, 0, 0, hWnd, ( HMENU )( INT_PTR )EDIT_ID, g_wv.hInstance, NULL );
	...
	s_wcd.SysInputLineWndProc = ( WNDPROC )( LONG_PTR )SetWindowLongPtrA( s_wcd.hwndInputLine, GWLP_WNDPROC, ( LONG_PTR )InputLineWndProc );
}
...
void Sys_CreateConsole()
{
	...
	s_wcd.hWnd = CreateWindowExA( exStyle, SYS_CONSOLE_CLASS, SYS_CONSOLE_TITLE, style, posX, posY, wndW, wndH, NULL, NULL, g_wv.hInstance, NULL );
...
void Sys_SetErrorText( const char* buf )
{
	...
	if( !s_wcd.hwndErrorBox ) {
		s_wcd.hwndErrorBox =
			CreateWindowExA( 0, "static", s_wcd.errorString, WS_CHILD | WS_VISIBLE | SS_SUNKEN | SS_LEFTNOWORDWRAP, 0, 0, 0, 0, s_wcd.hWnd, ( HMENU )( INT_PTR )ERRORBOX_ID, g_wv.hInstance, NULL );
