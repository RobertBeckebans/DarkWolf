...
inline void idList<type>::SetGranularity( int granularity )
{
	...
	if( m_list ) {
		...
		if( newsize != m_size ) { Resize( newsize ); }
	}
}
...
*/
template<class type>
inline void idList<type>::Condense()
{
	if( m_list ) {
		if( m_num ) {
			Resize( m_num );
...
{
	if( !m_list ) { Resize( m_granularity ); }
...
inline int idList<type>::AddUnique( type const& obj )
{
	...
	if( !Find( obj, &index ) ) { index = Append( obj ); }
...
inline bool idList<type>::RemoveIndex( int index )
{
	...
	if( !m_list || !m_num ) { return false; }

	assert( index >= 0 );
	assert( index < m_num );

	if( ( index < 0 ) || ( index >= m_num ) ) { return false; }
...
inline bool idList<type>::Remove( type const& obj )
{
	...
	if( Find( obj, &index ) ) { return RemoveIndex( index ); }
