...
public:
	...
	int selectPointByRay( const idVec3 origin, const idVec3 direction, bool single )
	{
		...
		if( besti >= 0 ) { selectPoint( besti, single ); }
	...
	int isPointSelected( int index )
	{
		...
		for( int i = 0; i < count; i++ ) {
			if( selectedPoints[i] == index ) { return i; }
		}
		return -1;
	}
	...
	int selectPoint( int index, bool single )
	{
		if( index >= 0 && index < numPoints() ) {
			if( single ) {
				...
				if( isPointSelected( index ) >= 0 ) { selectedPoints.Remove( index ); }
			}
...
public:
	...
	float		 getVelocity( long t )
	{
		...
		for( int i = 0; i < velocities.Num(); i++ ) {
			if( check >= velocities[i]->startTime && check <= velocities[i]->startTime + velocities[i]->time ) { return velocities[i]->speed; }
		}
		return baseVelocity;
	}
...
public:
	...
	{
		if( index != 0 ) { assert( true ); };
		return &pos;
	}

	virtual void draw( bool editMode ) { glLabeledPoint( blue, pos, ( editMode ) ? 5 : 3, "Fixed point" ); }
...
public:
	...
	virtual void		  addPoint( const float x, const float y, const float z ) { target.addPoint( x, y, z ); }
...
public:
	...
	{
		if( targetPositions.Num() == 0 ) { addTarget( NULL, idCameraPosition::FIXED ); }
	...
	int numPoints()
	{
		if( cameraEdit ) { return cameraPosition->numPoints(); }
		return getActiveTarget()->numPoints();
	}

	const idVec3* getPoint( int index )
	{
		if( cameraEdit ) { return cameraPosition->getPoint( index ); }
	...
	{
		if( cameraPosition == NULL ) { cameraPosition = new idFixedPosition(); }
		return cameraPosition;
	}
