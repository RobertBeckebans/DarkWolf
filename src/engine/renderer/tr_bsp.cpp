...
static void TryAddRectLightFromFace( const dsurface_t* ds, msurface_t* surf, srfSurfaceFace_t* cv )
{
	...
	glRaytracingLight_t light = glRaytracingLightingMakeRectLight( center[0],
		center[1],
		center[2],
		normal[0],
		normal[1],
...
static glRaytracingMeshHandle_t R_CreateRaytracingMeshFromFace( const srfSurfaceFace_t* face )
{
	...
	glRaytracingVertex_t* rtVerts = ( glRaytracingVertex_t* )ri.Hunk_AllocateTempMemory( sizeof( glRaytracingVertex_t ) * face->numPoints );

	uint32_t*			  rtIndices = ( uint32_t* )ri.Hunk_AllocateTempMemory( sizeof( uint32_t ) * face->numIndices );
...
static glRaytracingMeshHandle_t R_CreateRaytracingMeshFromTriSurf( const srfTriangles_t* tri )
{
	...
	glRaytracingVertex_t* rtVerts = ( glRaytracingVertex_t* )ri.Hunk_AllocateTempMemory( sizeof( glRaytracingVertex_t ) * tri->numVerts );

	uint32_t*			  rtIndices = ( uint32_t* )ri.Hunk_AllocateTempMemory( sizeof( uint32_t ) * tri->numIndexes );
...
static glRaytracingMeshHandle_t R_CreateRaytracingMeshFromGrid( const srfGridMesh_t* grid )
{
	...
	glRaytracingVertex_t* rtVerts = ( glRaytracingVertex_t* )ri.Hunk_AllocateTempMemory( sizeof( glRaytracingVertex_t ) * numVerts );

	uint32_t*			  rtIndices = ( uint32_t* )ri.Hunk_AllocateTempMemory( sizeof( uint32_t ) * numIndices );
...
// -----------------------------------------------------------------------------
static void R_CreateRaytracingWorldMeshes()
{
	int i;
	int count = 0;
	...
	s_bspRtMeshes = ( bspRaytracingMesh_t* )ri.Hunk_Alloc( sizeof( bspRaytracingMesh_t ) * s_worldData.numsurfaces, h_low );
	...
	for( i = 0; i < s_worldData.numsurfaces; ++i ) {
		...
		if( mesh ) {
			...
			instDesc.transform[0]  = 1.0f;
			instDesc.transform[1]  = 0.0f;
			instDesc.transform[2]  = 0.0f;
			instDesc.transform[3]  = 0.0f;
			instDesc.transform[4]  = 0.0f;
			instDesc.transform[5]  = 1.0f;
			instDesc.transform[6]  = 0.0f;
			instDesc.transform[7]  = 0.0f;
			instDesc.transform[8]  = 0.0f;
			instDesc.transform[9]  = 0.0f;
			instDesc.transform[10] = 1.0f;
			instDesc.transform[11] = 0.0f;
