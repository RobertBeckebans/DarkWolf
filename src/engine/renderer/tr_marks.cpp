...
=================
*/
int R_OldMarkFragments( int numPoints, const vec3_t* points, const vec3_t projection, int maxPoints, vec3_t pointBuffer, int maxFragments, markFragment_t* fragmentBuffer )
{
		...
		if( *surfaces[i] == SF_GRID ) {
			...
			if( !oldMapping ) {
				...
				for( k = 0; k < surf->numIndices; k += 3 ) {
					...
					// add the fragments of this face
					R_AddMarkFragments( 3, clipPoints, numPlanes, lnormals, ldists, maxPoints, pointBuffer, maxFragments, fragmentBuffer, &returnedPoints, &returnedFragments, lmins, lmaxs );
