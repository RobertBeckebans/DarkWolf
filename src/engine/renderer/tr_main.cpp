	...
	if( ent == NULL || ent->e.reType != RT_MODEL ) {
		if( viewParms ) {
			* or = viewParms->world;
			...
			or->modelMatrix[0]	= 1.0f;
			or->modelMatrix[4]	= 0.0f;
			or->modelMatrix[8]	= 0.0f;
			or->modelMatrix[12] = 0.0f;
			or->modelMatrix[1]	= 0.0f;
			or->modelMatrix[5]	= 1.0f;
			or->modelMatrix[9]	= 0.0f;
			or->modelMatrix[13] = 0.0f;
			or->modelMatrix[2]	= 0.0f;
			or->modelMatrix[6]	= 0.0f;
			or->modelMatrix[10] = 1.0f;
			or->modelMatrix[14] = 0.0f;
			or->modelMatrix[3]	= 0.0f;
			or->modelMatrix[7]	= 0.0f;
			or->modelMatrix[11] = 0.0f;
			or->modelMatrix[15] = 1.0f;
...
void R_AddDrawSurf( surfaceBase_t* surface, shader_t* shader, int fogIndex, int dlightMap, int atiTess, int dxrSurfaceId, trDXRMesh_t* dxrMesh, GeometryFlag_t flag )
{
	...
	tr.refdef.drawSurfs[index].sort =
		( shader->sortedIndex << QSORT_SHADERNUM_SHIFT ) | ( atiTess << QSORT_ATI_TESS_SHIFT ) | tr.shiftedEntityNum | ( fogIndex << QSORT_FOGNUM_SHIFT ) | ( int )dlightMap;
