...
void RB_DrawSun()
{
	...
	*/
	...
	if( r_drawSun->integer > 1 ) { // draw flare effect
		...
		// (SA) FIXME: todo: flare effect should render last (on top of everything else) and only when sun is in view (sun moving out of camera past degree n should start to cause flare dimming until
		// view angle to sun is off by angle n + x.
