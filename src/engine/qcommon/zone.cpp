...
void* Z_TagMalloc( int size, int tag )
{
	...
	do {
		if( rover == start ) {
			...
			//   Z_LogHeap();
#endif
			Com_Error( ERR_FATAL, "Z_Malloc: failed on allocation of %llu bytes from the %s zone", ( unsigned long long )totalSize, zone == smallzone ? "small" : "main" );
			return NULL;
		}
...
memstatic_t emptystring = { { MEMSTATIC_BLOCK_SIZE( 2 ), TAG_STATIC, NULL, NULL, ZONEID }, { '\0', '\0' } };

memstatic_t numberstring[] = { { { MEMSTATIC_T_SIZE, TAG_STATIC, NULL, NULL, ZONEID }, { '0', '\0' } },
...
void* Hunk_AllocateTempMemory( int size )
{
	...
	if( hunk_temp->temp + hunk_permanent->permanent + allocSize > s_hunkTotal ) {
		Com_Error( ERR_DROP, "Hunk_AllocateTempMemory: failed on %llu", ( unsigned long long )allocSize );
	}
