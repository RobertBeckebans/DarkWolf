...
static void setupQuad( long xOff, long yOff )
{
	...
	if( xOff == cin.oldXOff && yOff == cin.oldYOff && cinTable[currentHandle].ysize == ( unsigned int )cin.oldysize && cinTable[currentHandle].xsize == ( unsigned int )cin.oldxsize ) {
		return;
	}
	...
	switch( cinTable[currentHandle].roq_id ) {
		case ROQ_QUAD_VQ:
			...
			if( cinTable[currentHandle].numQuads == 0 ) {
				Com_Memcpy( cin.linbuf + cinTable[currentHandle].screenDelta, cin.linbuf, cinTable[currentHandle].samplesPerLine * ( long )cinTable[currentHandle].ysize );
			}
...
e_status CIN_RunCinematic( int handle )
{
	...
	cinTable[currentHandle].tfps = ( ( ( CL_ScaledMilliseconds() - ( int )cinTable[currentHandle].startTime ) * cinTable[currentHandle].roqFPS ) / 1000 );
