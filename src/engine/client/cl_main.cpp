...
{
	if( cl_allowDownload->integer && FS_ComparePaks( clc.downloadList, sizeof( clc.downloadList ), qfalse ) ) {
		if( *clc.downloadList ) {
...
static void CL_Cache_SetIndex_f()
{
	if( Cmd_Argc() < 2 ) {
...
static void CL_Cache_MapChange_f()
{
	cacheIndex++;
}

static void CL_Cache_EndGather_f()
{
