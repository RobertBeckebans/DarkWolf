...
public:
	...
	// game state
	virtual void LocateGameData( gentity_t* gEnts, int numGEntities, int sizeofGEntity_t, playerState_t* clients, int sizeofGClient ) override
	{
	...
	// collision / world
	virtual void Trace( trace_t* results, const vec3_t start, const vec3_t mins, const vec3_t maxs, const vec3_t end, int passEntityNum, int contentmask ) override
	{
		::SV_Trace( results, start, mins, maxs, end, passEntityNum, contentmask, qfalse );
	}

	virtual void TraceCapsule( trace_t* results, const vec3_t start, const vec3_t mins, const vec3_t maxs, const vec3_t end, int passEntityNum, int contentmask ) override
	{
	...
	// clients / commands
	virtual int BotAllocateClient() override
	{
	...
	// botlib core
	virtual int BotLibSetup() override
	{
		return ::SV_BotLibSetup();
	}

	virtual int BotLibShutdown() override
	{
	...
	virtual int AAS_Initialized() override
	{
	...
	virtual float AAS_Time() override
	{
	...
	virtual int AAS_PredictClientMovement( void* move,
		int										 entnum,
		vec3_t									 origin,
		int										 presencetype,
		int										 onground,
		...
		int										 visualize ) override
	{
		return ::botlib_export->aas.AAS_PredictClientMovement(
			static_cast<aas_clientmove_s*>( move ), entnum, origin, presencetype, onground, velocity, cmdmove, cmdframes, maxframes, frametime, stopevent, stopareanum, visualize );
	}
	...
	virtual qboolean AAS_RT_GetHidePos( vec3_t srcpos, int srcnum, int srcarea, vec3_t destpos, int destnum, int destarea, vec3_t returnPos ) override
	{
		return ::botlib_export->aas.AAS_RT_GetHidePos( srcpos, srcnum, srcarea, destpos, destnum, destarea, returnPos );
	}

	virtual int AAS_FindAttackSpotWithinRange( int srcnum, int rangenum, int enemynum, float rangedist, int travelflags, float* outpos ) override
	{
	...
	virtual int BotAllocChatState() override
	{
	...
	virtual void BotInitialChat( int chatstate, char* type, int mcontext, char* var0, char* var1, char* var2, char* var3, char* var4, char* var5, char* var6, char* var7 ) override
	{
	...
	virtual int BotReplyChat( int chatstate, char* message, int mcontext, int vcontext, char* var0, char* var1, char* var2, char* var3, char* var4, char* var5, char* var6, char* var7 ) override
	{
	...
	virtual int BotChooseNBGItem( int goalstate, vec3_t origin, int* inventory, int travelflags, void* ltg, float maxtime ) override
	{
		return ::botlib_export->ai.BotChooseNBGItem( goalstate, origin, inventory, travelflags, static_cast<bot_goal_t*>( ltg ), maxtime );
	}
	...
	virtual int BotItemGoalInVisButNotVisible( int viewer, vec3_t eye, vec3_t viewangles, void* goal ) override
	{
		return ::botlib_export->ai.BotItemGoalInVisButNotVisible( viewer, eye, viewangles, static_cast<bot_goal_t*>( goal ) );
	}
	...
	virtual void BotInitLevelItems() override
	{
		::botlib_export->ai.BotInitLevelItems();
	}

	virtual void BotUpdateEntityItems() override
	{
	...
	{
		::botlib_export->ai.BotMoveToGoal( static_cast<bot_moveresult_t*>( result ), movestate, static_cast<bot_goal_t*>( goal ), travelflags );
	}
	...
	virtual int BotMovementViewTarget( int movestate, void* goal, int travelflags, float lookahead, vec3_t target ) override
	{
		return ::botlib_export->ai.BotMovementViewTarget( movestate, static_cast<bot_goal_t*>( goal ), travelflags, lookahead, target );
	}

	virtual int BotPredictVisiblePosition( vec3_t origin, int areanum, void* goal, int travelflags, vec3_t target ) override
	{
		return ::botlib_export->ai.BotPredictVisiblePosition( origin, areanum, static_cast<bot_goal_t*>( goal ), travelflags, target );
	}

	virtual int BotAllocMoveState() override
	{
	...
	virtual int BotAllocWeaponState() override
	{
