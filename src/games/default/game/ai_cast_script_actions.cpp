		...
		if( moverange > 0 ) {
			...
			if( ( cs->castScriptStatus.scriptWaitMovetime < level.time ) && ( cs->enemyNum >= 0 ) ) {
				...
				if( AICast_CheckAttack( cs, cs->enemyNum, qfalse ) ||
					( !AICast_EntityVisible( AICast_GetCastState( cs->enemyNum ), cs->entityNum, qfalse ) && !AICast_CheckAttack( AICast_GetCastState( cs->enemyNum ), cs->entityNum, qfalse ) ) ) {
