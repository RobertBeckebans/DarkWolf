...
extern void AICast_CastScriptThink();
...
void G_RunFrame( int levelTime )
{
	...
	for( i = 0; i < level.num_entities; i++, ent++ ) {
		...
		if( ent->s.eType == ET_MISSILE || ent->s.eType == ET_FLAMEBARREL || ent->s.eType == ET_FP_PARTS || ent->s.eType == ET_FIRE_COLUMN || ent->s.eType == ET_FIRE_COLUMN_SMOKE ||
			ent->s.eType == ET_EXPLO_PART || ent->s.eType == ET_RAMJET ) {
			G_RunMissile( ent );
			continue;
		}
