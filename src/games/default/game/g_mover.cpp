...
void SetMoverState( gentity_t* ent, moverState_t moverState, int time )
{
	...
	if( !( ent->r.svFlags & SVF_NOCLIENT ) ||
		( ent->r.contents ) ) { // RF, added this for bats, but this is safe for all movers, since if they aren't solid, and aren't visible to the client, they don't need to be linked
		sys->LinkEntity( ent );
...
doors). NOMONSTER   monsters will not trigger this door SHOOT-THRU	Bullets don't stop when they hit the door.  Set "shoot_thru_scale" with bullet damage scale (see below)
...
void Static_Pain( gentity_t* ent, gentity_t* attacker, int damage, vec3_t point )
{
	...
	if( ent->spawnflags & 4 ) {
		...
		if( attacker && attacker->client && ( attacker->s.weapon == WP_VENOM || attacker->s.weapon == WP_GRENADE_LAUNCHER || attacker->client->ps.persistant[PERS_HWEAPON_USE] ) ) {
...
You need to have an origin brush as part of this entity.
The center of that brush will be the point around which it is rotated. It will rotate around the Z axis by default.  You can check either the X_AXIS or Y_AXIS box to change that (only one axis
allowed. If both X and Y are checked, the default of Z will be used). FORCE		door opens even if blocked
...
"mass" - defaults to 75.  This determines how much debris is emitted when it explodes.  You get one large chunk per 100 of mass (up to 8) and one small chunk per 25 of mass (up to 16).  So 800 gives
the most. "noise" - sound to play when triggered.  The explosive will default to a sound that matches it's 'type'.  Use the sound name "nosound" (case in-sensitive) if you want it silent. "leaktype" -
leaks particles of this type "leakpressure" - force the particles come out with.  '0' would cause them to fall straight down "leaktime" - how long it leaks before it quits "leakcount" - how many holes
in the entity before it no longer leaks. the default sounds are: "wood"	- "sound/world/boardbreak.wav" "glass"	- "sound/world/glassbreak.wav" "metal"	- "sound/world/metalbreak.wav" "gibs"	-
"sound/player/gibsplit1.wav" "brick"	- "sound/world/brickfall.wav" "stone"	- "sound/world/stonefall.wav" "fabric"	- "sound/world/metalbreak.wav"	// (SA) temp 'leaktypes': 1:oil 2:water 3:steam
4:wine
5:smoke
6:electrical
*/
