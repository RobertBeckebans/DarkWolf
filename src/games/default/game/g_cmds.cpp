...
void DeathmatchScoreboardMessage( gentity_t* ent )
{
	...
	sys->SendServerCommand( ent - g_entities, va( "scores %i %i %i%s", i, level.teamScores[TEAM_RED], level.teamScores[TEAM_BLUE], string ) );
}
...
void Cmd_LevelShot_f( gentity_t* ent )
{
	...
	if( g_gametype.integer != 0 ) {
		sys->SendServerCommand( ent - g_entities, "print \"Must be in g_gametype 0 for levelshot\n\"" );
		return;
	}
...
void ClientDamage( gentity_t* clent, int entnum, int enemynum, int id )
{
	...
	switch( id ) {
		...
		case CLDMG_TESLA:
			...
			if( ( ent->aiCharacter == AICHAR_PROTOSOLDIER ) || ( ent->aiCharacter == AICHAR_SUPERSOLDIER ) || ( ent->aiCharacter == AICHAR_LOPER ) ||
