...
void BotChooseWeapon( bot_state_t* bs )
{
	...
	if( bs->cur_ps.weaponstate == WEAPON_RAISING || bs->cur_ps.weaponstate == WEAPON_RAISING_TORELOAD ||	//----(SA)	added
		bs->cur_ps.weaponstate == WEAPON_DROPPING || bs->cur_ps.weaponstate == WEAPON_DROPPING_TORELOAD ) { //----(SA)	added
...
float BotAggression( bot_state_t* bs )
{
	...
	if( bs->inventory[INVENTORY_SP5] > 0 && bs->inventory[INVENTORY_SP5AMMO] > 40 ) {
		return 85;
	}
...
int BotFindEnemy( bot_state_t* bs, int curenemy )
{
	...
	for( i = 0; i < MAX_CLIENTS; i++ ) {
		...
		if( curenemy >= 0 ) {
			...
			bs->enemysight_time = sys->AAS_Time();
		}
		...
		if( wi.speed ) {
			...
			if( !( dist > 100 && VectorLength( dir ) < 32 ) ) {
				...
				if( aim_skill > 0.8 &&
					...
					sys->AAS_PredictClientMovement( &move, bs->enemy, origin, PRESENCE_CROUCH, qfalse, dir, cmdmove, 0, dist * 10 / wi.speed, 0.1, 0, 0, qfalse );
...
void BotCheckAttack( bot_state_t* bs )
{
	...
	BotAI_Trace( &trace,
		...
		bs->entitynum,
		MASK_SHOT ); //----(SA) should this maybe check the weapon type and adjust the clipflag?  it seems like this is probably fine as-is, but I thought I'd note it.
		...
		if( !BotMatchMessage( bs, m.message ) ) {
			...
			if( m.type == CMS_CHAT && !bot_nochat.integer ) {
				...
				if( bot_testrchat.integer ) {
					...
					if( sys->BotReplyChat( bs->cs, message, context, CONTEXT_REPLY, NULL, NULL, NULL, NULL, NULL, NULL, botname, netname ) ) {
