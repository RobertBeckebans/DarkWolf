...
void G_RunMissile( gentity_t* ent )
{
	...
	sys->Trace( &tr, ent->r.currentOrigin, ent->r.mins, ent->r.maxs, origin, ent->r.ownerNum, ent->clipmask );
...
int G_PredictMissile( gentity_t* ent, int duration, vec3_t endPos, qboolean allowBounce )
{
	...
	for( time = level.time + FRAMETIME; time < level.time + duration; time += FRAMETIME ) {
		...
		// trace a line from the previous position to the current position,
		// ignoring interactions with the missile owner
		sys->Trace( &tr, org, ent->r.mins, ent->r.maxs, origin, ent->r.ownerNum, ent->clipmask );
...
void G_RunSpit( gentity_t* ent )
{
	...
	if( rand() % 100 > 60 ) {
		...
		sys->Trace( &tr, ent->r.currentOrigin, NULL, NULL, end, ent->r.ownerNum, MASK_SHOT );
...
void fire_lead( gentity_t* self, vec3_t start, vec3_t dir, int damage )
{
	...
	if( traceEnt->takedamage ) {
		if( self->s.weapon == WP_SNIPER && traceEnt->s.eType == ET_MOVER && traceEnt->aiName[0] ) {
