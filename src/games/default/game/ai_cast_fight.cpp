...
{
	if( cs->bs ) {
		if( ( cs->checkAttackCache.time == level.time ) && ( cs->checkAttackCache.enemy == enemy ) && ( cs->checkAttackCache.weapon == cs->weaponNum ) &&
				...
				case AICHAR_LOPER:													// loper leap attack
					if( cs->bs->areanum && VectorLength( cs->bs->velocity ) > 1 ) { // if we are in a valid area, and are persuing, then leave a delay
						...
						if( cs->enemyNum >= 0 ) {
							...
							mins[0] = 0;
							sys->Trace( &trace,
								g_entities[cs->entityNum].client->ps.origin,
								mins,
								cs->bs->cur_ps.maxs,
...
void AICast_ChooseWeapon( cast_state_t* cs, qboolean battleFunc )
{
	...
	if( cs->bs->cur_ps.weaponstate == WEAPON_RAISING || cs->bs->cur_ps.weaponstate == WEAPON_RAISING_TORELOAD ||	//----(SA)	added
		cs->bs->cur_ps.weaponstate == WEAPON_DROPPING || cs->bs->cur_ps.weaponstate == WEAPON_DROPPING_TORELOAD ) { //----(SA)	added
		return;
	}
...
int AICast_WantsToTakeCover( cast_state_t* cs, qboolean attacking )
{
	...
	if( cs->attributes[AGGRESSION] < 1.0 && attacking && ( cs->enemyNum >= 0 ) && ( g_entities[cs->enemyNum].client->ps.weapon ) && ( cs->attributes[TACTICAL] > 0.5 ) &&
		( cs->aiFlags & AIFL_ROLL_ANIM ) && ( VectorLength( cs->bs->cur_ps.velocity ) < 1 ) ) {
		vec3_t aim, enemyVec;
...
qboolean AICast_AimAtEnemy( cast_state_t* cs )
{
	...
	if( dist > 256 ) {
		VectorMA( bestorigin,
			( 0.3 + 0.7 * ( 1 - aim_accuracy ) ) * 0.4 * sin( ( float )level.time / ( 500.0 + ( 100.0 * ( ( cs->entityNum + 3 ) % 4 ) ) ) ),
...
qboolean AICast_WantToRetreat( cast_state_t* cs )
{
	...
	if( cs->leaderNum < 0 ) {
		if( ( cs->attributes[TACTICAL] > 0.11 + random() * 0.5 ) &&
			( ( cs->bs->cur_ps.weaponTime > 500 ) || ( ( cs->takeCoverTime < level.time - 100 ) && ( AICast_WantsToTakeCover( cs, qtrue ) ) ) ) ) {
			return qtrue;
		}
	}
