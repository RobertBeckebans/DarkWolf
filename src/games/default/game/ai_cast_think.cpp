...
void AICast_ChangeViewAngles( cast_state_t* cs, float thinktime )
{
	...
	if( cs->lockViewAnglesTime < level.time ) {
		...
		for( i = 0; i < 3; i++ ) {
			...
			cs->viewangles[i] = BotChangeViewAngle( cs->viewangles[i], cs->ideal_viewangles[i], anglespeed );
...
void	   AICast_InputToUserCommand( cast_state_t* cs, bot_input_t* bi, usercmd_t* ucmd, int delta_angles[3] )
{
	...
	if( bi->actionflags & ACTION_ATTACK ) {
		...
		if( ( cs->triggerReleaseTime < level.time ) && ( ( cs->lockViewAnglesTime >= level.time ) || ( fabs( AngleDifference( cs->ideal_viewangles[YAW], cs->viewangles[YAW] ) ) < 20 ) ) &&
			// check for radid luger firing by skilled users (release fire between shots)
			( ( ( level.time + cs->entityNum * 500 ) / 2000 ) % 2 || !( rand() % ( 1 + g_gameskill.integer ) ) || ( cs->attributes[ATTACK_SKILL] < 0.5 ) || ( cs->weaponNum != WP_LUGER ) ||
	...
	if( bi->speed ) {
		if( ( !( cs->aiFlags & AIFL_WALKFORWARD ) && cs->enemyNum >= 0 ) ||
			( ( ucmd->forwardmove >= 0 ) && fabs( AngleNormalize180( AngleDifference( cs->ideal_viewangles[YAW], cs->viewangles[YAW] ) ) ) < 60 ) ) {
		...
		if( cs->bs ) {
			if( ent->aiInactive == qfalse && ent->inuse ) {
				...
				if( cs->lastThink && elapsed > 0 ) {
					...
					if( ( highPriority && ( elapsed > 300 ) ) || ( g_entities[0].client && g_entities[0].client->cameraPortal ) ||
						( highPriority && ( cs->vislist[0].visible_timestamp == cs->vislist[0].lastcheck_timestamp ) ) ||
						( highPriority && ( pcs->vislist[cs->entityNum].visible_timestamp == pcs->vislist[cs->entityNum].lastcheck_timestamp ) ) || ( VectorLength( ent->client->ps.velocity ) > 0 ) ||
...
void AICast_PredictMovement( cast_state_t* cs, int numframes, float frametime, aicast_predictmove_t* move, usercmd_t* ucmd, int checkHitEnt )
{
	...
	for( frame = 0; frame < numframes; frame++ ) {
		...
		pm.trace = []( trace_t* results, const vec3_t start, const vec3_t mins, const vec3_t maxs, const vec3_t end, int passEntityNum, int contentMask ) {
			sys->Trace( results, start, mins, maxs, end, passEntityNum, contentMask );
		};

		pm.pointcontents = []( const vec3_t point, int passEntityNum ) -> int { return sys->PointContents( point, passEntityNum ); };
...
qboolean AICast_GetAvoid( cast_state_t* cs, bot_goal_t* goal, vec3_t outpos, qboolean reverse, int blockEnt )
{
	...
	enemyVisible = ( qboolean )( ( cs->enemyNum >= 0 ) && ( AICast_CheckAttack( cs, cs->enemyNum, qfalse ) ) );
	//
...
void AICast_Blocked( cast_state_t* cs, bot_moveresult_t* moveresult, int activate, bot_goal_t* goal )
{
	...
	if( cs->blockedAvoidTime < level.time ) {
		...
		if( g_entities[blockEnt].aiName && g_entities[blockEnt].client && cs->bs && VectorDistance( cs->bs->origin, g_entities[blockEnt].r.currentOrigin ) < 128 ) {
...
void AICast_EvaluatePmove( int clientnum, pmove_t* pm )
{
	...
	for( i = 0; i < pm->numtouch; i++ ) {
		...
		if( pm->touchents[i] < aicast_maxclients ) {
			...
			if( ( ocs->bs ) && ( AICast_SameTeam( cs, ocs->entityNum ) ) && ( !( ocs->aiFlags & AIFL_NOAVOID ) ) &&
				( ( ocs->leaderNum == cs->entityNum ) || ( VectorLength( ocs->bs->velocity ) < 5 ) ) && ( ocs->obstructingTime < ( level.time + 100 ) ) ) {
				// if they are moving away from us already, let them go
				if( VectorLength( ocs->bs->cur_ps.velocity ) > 10 ) {
					vec3_t v1, v2;
			...
			if( g_entities[ent].s.modelindex > 0 && g_entities[ent].s.eType == ET_MOVER ) {
				...
				if( !g_entities[ent].isProp && Q_stricmp( g_entities[ent].classname, "func_static" ) && Q_stricmp( g_entities[ent].classname, "func_button" ) &&
...
void AICast_IdleReload( cast_state_t* cs )
{
	...
	if( !( ( cs->bs->cur_ps.ammoclip[BG_FindClipForWeapon( ( weapon_t )cs->bs->cur_ps.weapon )] < ( int )( 0.75 * ammoTable[cs->bs->cur_ps.weapon].maxclip ) ) &&
			cs->bs->cur_ps.ammo[BG_FindAmmoForWeapon( ( weapon_t )cs->bs->cur_ps.weapon )] ) ) {
		return;
	}
	//
