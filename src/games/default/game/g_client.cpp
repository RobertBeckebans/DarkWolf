	...
	while( !finished ) {
		...
		for( i = 1; i < numDeployable; i++ )
			if( level.clients[deployable[i]].deployQueueNumber < level.clients[deployable[i - 1]].deployQueueNumber ) {
				G_Printf(
					"swapping %d and %d (priority %d and %d)\n", deployable[i], deployable[i - 1], level.clients[deployable[i]].deployQueueNumber, level.clients[deployable[i - 1]].deployQueueNumber );
				j				  = deployable[i - 1];
