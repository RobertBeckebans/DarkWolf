...
cast_script_stack_action_t scriptActions[] = { { "gotomarker", AICast_ScriptAction_GotoMarker },
...
cast_script_event_define_t scriptEvents[] = { { "spawn", NULL }, // called as each character is spawned into the game
			...
			while( ( token = COM_Parse( &pScript ) ) && ( token[0] != '}' ) ) {
				...
				for( i = 0; token[0]; i++ ) {
					...
					if( i == 0 ) {
						...
						//----(SA)	added a bit more
						if( buildScript && ( !Q_stricmp( action->actionString, "mu_start" ) || !Q_stricmp( action->actionString, "mu_play" ) || !Q_stricmp( action->actionString, "mu_queue" ) ||
...
void AICast_ScriptEvent( struct cast_state_s* cs, char* eventStr, char* params )
{
	...
	// show debugging info
	if( ( ( aicast_debug.integer == 1 ) || ( ( aicast_debug.integer == 2 ) && ( ( strlen( aicast_debugname.string ) < 1 ) ||
																				  ( g_entities[cs->entityNum].aiName && !strcmp( aicast_debugname.string, g_entities[cs->entityNum].aiName ) ) ) ) ) ) {
