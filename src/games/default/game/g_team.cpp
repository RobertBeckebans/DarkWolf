...
void Team_FragBonuses( gentity_t* targ, gentity_t* inflictor, gentity_t* attacker )
{
	...
	if( ( VectorLength( v1 ) < CTF_TARGET_PROTECT_RADIUS || VectorLength( v2 ) < CTF_TARGET_PROTECT_RADIUS || CanDamage( flag, targ->s.origin ) || CanDamage( flag, attacker->s.origin ) ) &&
	...
	if( carrier && carrier != attacker ) {
		...
		if( VectorLength( v1 ) < CTF_ATTACKER_PROTECT_RADIUS || VectorLength( v2 ) < CTF_ATTACKER_PROTECT_RADIUS || CanDamage( carrier, targ->s.origin ) || CanDamage( carrier, attacker->s.origin ) ) {
...
void Team_ReturnFlagSound( gentity_t* ent, int team )
{
	...
	te->s.eventParm = G_SoundIndex( team == TEAM_RED ? "sound/teamplay/flagret_red.wav" : "sound/teamplay/flagret_blu.wav" );
...
int Team_TouchOurFlag( gentity_t* ent, gentity_t* other, int team )
{
	...
	te				= G_TempEntity( ent->s.pos.trBase, EV_GLOBAL_SOUND );
	te->s.eventParm = G_SoundIndex( our_flag == PW_REDFLAG ? "sound/teamplay/flagcap_red.wav" : "sound/teamplay/flagcap_blu.wav" );
	te->r.svFlags |= SVF_BROADCAST;
