...
void AICast_UpdateVisibility( gentity_t* srcent, gentity_t* destent, qboolean shareVis, qboolean directview )
{
	...
	if( ( destent->health > 0 ) && ( srcent->aiTeam == destent->aiTeam ) && // only share with exact same team, and non-neutrals
		...
		for( i = 0; i < aicast_maxclients && cnt < level.numPlayingClients; i++ ) {
			...
			if( destent->health > 0 && shareRange ) {
				...
				if( ( ovis->visible_timestamp > svis->visible_timestamp ) || ( ( ovis->visible_timestamp > level.time - 5000 ) && ( ovis->flags & AIVIS_ENEMY ) && !( svis->flags & AIVIS_ENEMY ) ) ) {
					// trigger an EVENT
...
#define SIGHT_MIN_DELAY 200
	...
	for( srccount = 0, src = 0, srcent = &g_entities[0]; src < aicast_maxclients && srccount < level.numPlayingClients; src++, srcent++ ) {
		if( !srcent->inuse ) {
			continue;
		}
		...
		for( destcount = 0, dest = 0, destent = g_entities;
			...
			if( !( destent->flags & FL_NOTARGET ) && ( AICast_CheckVisibility( srcent, destent ) ) ) {
				// record the sighting
	...
	for( srccount = 0, src = lastsrc, srcent = &g_entities[lastsrc]; src < aicast_maxclients; // && srccount < level.numPlayingClients;
		 src++, srcent++ ) {
		if( !srcent->inuse ) {
			continue;
		}
		...
		for( destcount = 0, dest = lastdest, destent = &g_entities[lastdest]; dest < aicast_maxclients; // && destcount < level.numPlayingClients;
			 dest++, destent++ ) {
			if( !destent->inuse ) {
				continue;
			}
			...
			if( AICast_SameTeam( cs, destent->s.number ) && ( vis->lastcheck_timestamp == vis->visible_timestamp ) && ( destent->health == vis->lastcheck_health + 1 ) ) {
