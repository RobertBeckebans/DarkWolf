...
==============
*/
static void CG_ParseMissionStats()
{
	const char* info;
	char*		token;
...
=====================
*/
static void CG_ParseScreenFade()
{
	const char* info;
	char*		token;
	float		fadealpha;
...
static void CG_ServerCommand()
{
	...
	if( !strcmp( cmd, "dp" ) ) { // dynamite print (what a hack :(
		...
		CG_CenterPrint( va( "%s %d %s", CG_translateString( "dynamitetimer" ), atoi( CG_Argv( 1 ) ), CG_translateString( "seconds" ) ), SCREEN_HEIGHT - ( SCREEN_HEIGHT * 0.25 ), SMALLCHAR_WIDTH );
		return;
	}
