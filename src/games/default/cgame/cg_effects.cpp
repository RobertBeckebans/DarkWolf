			...
			for( j = 0; j < MAXJUNCTIONS; j++ ) {
				if( !Q_stricmp( JunctiongibTags[j], ConnectTags[i] ) ) {
					if( newjunction[j] == qtrue ) {
						...
						if( ( cent->currentState.aiChar == AICHAR_ZOMBIE ) || ( cent->currentState.aiChar == AICHAR_HEINRICH ) || ( cent->currentState.aiChar == AICHAR_HELGA ) ) {
							// spawn a gib
...
unsigned short randtable[NUMRANDTABLE] = { 0x0000,
	0x1021,
	0x2042,
	0x3063,
	0x4084,
	...
	for( i = 0; i < numBolts; i++ ) {
		...
		while( distLeft > 0 ) {
			...
			if( thisWidth < 4 && distLeft > 10 && recursion < 3 && forks < 3 && lt_random( randseed, 383 + i + forks ) < FORK_CHANCE ) {
				...
				forks++;
				VectorSet(
					fend, distLeft * 0.3 * lt_crandom( randseed, 56 + i + forks ), distLeft * 0.3 * lt_crandom( randseed, 160 + i + forks ), distLeft * 0.3 * lt_crandom( randseed, 190 + i + forks ) );
