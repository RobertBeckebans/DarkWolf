...
static qboolean CG_RegisterClientModelname( clientInfo_t* ci, const char* modelName, const char* skinName )
{
	...
	//----(SA)	testing
	{
		...
		if( badscale ) {
			ci->playermodelScale[0] = ci->playermodelScale[1] = ci->playermodelScale[2] = 0.0f;
		}
	}
...
static qboolean CG_ScanForExistingClientInfo( clientInfo_t* ci )
{
	...
	for( i = 0; i < cgs.maxclients; i++ ) {
		...
		if( !Q_stricmp( ci->modelName, match->modelName ) && !Q_stricmp( ci->skinName, match->skinName ) && !Q_stricmp( ci->hSkinName, match->hSkinName ) ) {
			//----(SA) done
	...
	*/
	...
	if( !CG_ScanForExistingClientInfo( &newInfo ) ) {
		...
		if( forceDefer ) {
			...
			if( ci->infoValid && ( cgs.gametype < GT_TEAM || !Q_stricmp( newInfo.skinName, ci->skinName ) ) ) {
...
#define ANIM_SPEEDMAX_HIGH 20
	...
	if( newAnimation != lf->animationNumber || !lf->animation ) {
		...
		CG_Printf( "(%i) %s anim change on %s: %s -> %s\n", cg.time, ci->modelInfo->modelname, ( lf == &cent->pe.legs ? "LEGS" : "TORSO" ), lf->animation->name,
				   ci->modelInfo->animations[newAnimation & ~( 1 << 9 )].name );
	#endif
#endif
		...
		if( lf->animSpeedScale < 0.25 ) { // if it's too slow, then a really slow spped, combined with a sudden take-off, can leave them playing a really slow frame while they a moving really fast
			...
			if( !( anim->flags & ANIMFL_LADDERANIM ) ) {
				...
				if( anim->moveSpeed > ANIM_SPEEDMAX_LOW ) {
					...
					lf->animSpeedScale =
						ANIM_SCALEMAX_HIGH - ( ANIM_SCALEMAX_HIGH - ANIM_SCALEMAX_LOW ) * ( float )( anim->moveSpeed - ANIM_SPEEDMAX_HIGH ) / ( float )( ANIM_SPEEDMAX_LOW - ANIM_SPEEDMAX_HIGH );
				}
		...
		if( !lf->animSpeedScale ) {
			...
			lf->frameTime += anim->frameLerp; // don't wait too long before starting to move again
				   ...
				   ( !anim->moveSpeed || lf->oldFrame < anim->firstFrame ||
					   lf->oldFrame >= anim->firstFrame +
										   anim->numFrames ) ) { // Ridah, added this so walking frames don't always get reset to 0, which can happen in the middle of a walking anim, which looks wierd
			lf->frameTime = lf->animationTime;					 // initial lerp
			...
			if( oldAnim && anim->moveSpeed ) { // keep locomotions going continuously
				...
				f = 0;
				   ...
				   ( ( lf->animationNumber & ~ANIM_TOGGLEBIT ) == ( cent->pe.torso.animationNumber & ~ANIM_TOGGLEBIT ) ) && ( !anim->moveSpeed ) ) {
			...
			lf->frameTime = cent->pe.torso.frameTime;
				   ...
				   ( ( lf->animationNumber & ~ANIM_TOGGLEBIT ) == ( cent->pe.legs.animationNumber & ~ANIM_TOGGLEBIT ) ) && ( otherAnim->moveSpeed ) ) {
...
static void CG_PlayerSprites( centity_t* cent )
{
	...
	if( cgs.gametype == GT_WOLF && ( cent->currentState.eFlags & EF_DEAD ) && cent->currentState.number == cent->currentState.clientNum && cg.snap->ps.stats[STAT_PLAYER_CLASS] == PC_MEDIC &&
	...
	if( cgs.gametype != GT_WOLF && cgs.gametype >= GT_TEAM && !( cent->currentState.eFlags & EF_DEAD ) && cg.snap->ps.persistant[PERS_TEAM] == team ) {
...
#define SHADOW_MAX_DIST 512.0
	shadowPart_t shadowParts[] = { { "tag_footleft", 10, 4, 0.5, 0 }, { "tag_footright", 10, 4, 0.5, 0 }, { "tag_torso", 18, 96, 0.4, 0 }, { NULL, 0 } };
	...
	*shadowPlane = trace.endpos[2] + 1;
	...
	if( dist > SHADOW_MIN_DIST ) {
		...
		alpha *= distfade;
		CG_ImpactMark( cgs.media.shadowTorsoShader, trace.endpos, trace.plane.normal, 0, alpha, alpha, alpha, 1, qfalse, 18 * scale, qtrue, -1 );
	...
	if( dist < SHADOW_MAX_DIST ) { // show more detail
		...
		for( tagIndex = 0; shadowParts[tagIndex].tagname; tagIndex++ ) {
			...
			for( subIndex = 0; ( subIndex = CG_GetOriginForTag( cent, &cent->pe.legsRefEnt, shadowParts[tagIndex].tagname, subIndex, origin, axis ) ) >= 0; subIndex++ ) {
				...
				CG_ImpactMark(
					shadowParts[tagIndex].shader, origin, trace.plane.normal, angles[YAW] /*cent->pe.legs.yawAngle*/, alpha, alpha, alpha, 1, qfalse, shadowParts[tagIndex].size * scale, qtrue, -1 );
			}
		}
	}
	...
	for( t = cent->pe.lastZombieSpirit + step; t <= cg.time; t += step ) {
		...
		for( i = 0; i < MAX_ZOMBIE_SPIRITS; i++ ) {
			...
			if( cent->pe.cueZombieSpirit ) {
				...
				radius = ( minDist + sin( M_PI * 2 * ( float )( ( float )( ( int )( t + ( ( float )( radiusCycleTime * i ) / MAX_ZOMBIE_SPIRITS ) ) % radiusCycleTime ) / ( float )radiusCycleTime ) ) *
										 ( maxDist - minDist ) );

				// get the position
				v[0] = ( 0.5 + 0.5 * fadeRatio ) *
					   sin( M_PI * 2 * ( float )( ( float )( ( int )( t + ( ( float )( rotationTime * i ) / MAX_ZOMBIE_SPIRITS ) ) % rotationTime ) / ( float )rotationTime ) ) * radius;
				v[1] = ( 0.5 + 0.5 * fadeRatio ) *
					   cos( M_PI * 2 * ( float )( ( float )( ( int )( t + ( ( float )( rotationTime * i ) / MAX_ZOMBIE_SPIRITS ) ) % rotationTime ) / ( float )rotationTime ) ) * radius;
				v[2] = 12 + 36 * ( 0.5 + 0.5 * fadeRatio ) *
								sin( M_PI * 2 * ( float )( ( float )( ( int )( t + ( ( float )( zCycleTime * i ) / MAX_ZOMBIE_SPIRITS ) ) % zCycleTime ) / ( float )zCycleTime ) );
				...
				radius =
					...
					( 1.0 - pow( 1.0 - fadeRatio, 2 ) ) *
						( minDist + sin( M_PI * 2 * ( float )( ( float )( ( int )( t + ( ( float )( radiusCycleTime * i ) / MAX_ZOMBIE_SPIRITS ) ) % radiusCycleTime ) / ( float )radiusCycleTime ) ) *
										( maxDist - minDist ) );
...
#define LOPER_MAX_POINT_TESTS_PERFRAME 20
	...
	for( i = 0; i < numPoints; i++ ) {
		...
		if( ( cent->currentState.eFlags & EF_MONSTER_EFFECT ) && ( ( !cent->pe.lightningTimes[i] ) || ( cent->pe.lightningTimes[i] > cg.time ) || ( cent->pe.lightningTimes[i] < cg.time - 50 ) ||
		...
		} else if( ( !cent->pe.lightningTimes[i] ) || ( cent->pe.lightningTimes[i] > cg.time ) || ( cent->pe.lightningTimes[i] < cg.time - LOPER_LIGHTNING_POINT_TIMEOUT ) ||
				   ( VectorDistance( cent->lerpOrigin, cent->pe.lightningPoints[i] ) > maxDist ) ) {
			...
			for( j = 0; j < LOPER_MAX_POINT_TESTS; j++ ) {
				VectorSet( testPos, maxDist * crandom(), maxDist * crandom(), maxDist * ( crandom() - 0.5 ) );
		...
		if( VectorDistance( cent->lerpOrigin, cent->pe.lightningPoints[i] ) <= maxDist ) {
			CG_DynamicLightningBolt(
				cgs.media.lightningBoltShader, tagPos, cent->pe.lightningPoints[i], 1, 25 + 12.0 * random(), ( cent->currentState.eFlags & EF_MONSTER_EFFECT ) == 0, 1.0, 0, i * i * 2 );
		}
	}
}
...
#define LOPER_GROUNDCHARGE_RADIUS	150
	...
	VectorScale( c, alpha, c );
	// add the light
	sys->R_AddLightToScene(
		cent->lerpOrigin, LOPER_GROUNDCHARGE_RADIUS * ( 3.0 + 2.0 * ( 1.0 + sin( 0.001 * ( ( cg.time ) % ( 1000 * ( 2 + cent->currentState.number ) ) ) ) ) / 2.0 ), c[0], c[1], c[2], 1 );
				...
				if (cent->pe.zombieDeathFadeEnd + fadeRiseTime > cg.time) {
					...
					if (cent->pe.zombieDeathFadeEnd > cg.time) {
						...
						if (cent->pe.zombieDeathFadeStart > cg.time) {
							...
							ent->shaderRGBA[3] = 128 - (unsigned char)(128.0*pow(((float)(cg.time - cent->pe.zombieDeathFadeStart) / (float)(cent->pe.zombieDeathFadeEnd -
		   cent->pe.zombieDeathFadeStart)), 2));
						}
						ent->shaderTime = 1.0;
...
#define NUM_EMOTIONS 2	 // 0 neutral, 1 happy, 2 angry
		...
		if( ( cent->pe.head.frameTime > cg.time + 30 ) && cent->pe.head.animationTime && !( ( cent->pe.head.frame - ci->modelInfo->headAnims[0].firstFrame ) % HEAD_EMOTION_SUBTYPES ) &&
			!( ( cent->pe.head.oldFrame - ci->modelInfo->headAnims[0].firstFrame ) % HEAD_EMOTION_SUBTYPES ) ) {
	...
	//
	if( ( cent->currentState.clientNum != cg.snap->ps.clientNum ) && cent->currentState.eFlags & EF_ZOOMING ) {
	...
	if( ( cent->currentState.clientNum != cg.snap->ps.clientNum ) && cent->currentState.eFlags & EF_CIG ) {
	...
	} else if( cent->currentState.aiChar == AICHAR_WARZOMBIE && ( !Q_strcasecmp( ( char* )ci->modelInfo->modelname, "dark" ) ) ) {
		// TTimo: unused
		/*
