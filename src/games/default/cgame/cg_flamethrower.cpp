...
void	   CG_FlameDamage( int owner, vec3_t org, float radius )
{
	...
	if( cgs.localServer && cgs.gametype == GT_SINGLE_PLAYER ) {
		...
		for( cent = cg_entities, i = 0; i < cgs.maxclients; cent++, i++ ) {
			if( cent->currentState.aiChar && cent->currentState.aiChar != AICHAR_VENOM &&  // Venom has no flame damage
				centFlameInfo[i].lastDmgCheck < ( cg.time - 100 ) && cent->currentValid && // is in the visible frame
...
#define FLAME_SOUND_RANGE 1024.0
	...
	if( ( cg_entities[fHead->ownerCent].currentState.eFlags & EF_FIRING ) /*(centFlameInfo[fHead->ownerCent].lastClientFrame == cg_entities[fHead->ownerCent].lastWeaponClientFrame)*/
...
#define FLAME_SPRITE_START_BLUE_SCALE 0.2
		...
		if( !f->ignitionOnly &&
			...
			while( fNext && !droppedTrail ) {
				if( ( Distance( f->org, fNext->org ) < ( ( 0.2 + 0.8 * f->lifeFrac ) * f->size * ( isClientFlame ? 0.2 : 0.1 ) ) ) && ( fabs( f->size - fNext->size ) < ( 40.0 ) ) &&
					( fabs( f->timeStart - fNext->timeStart ) < 50 ) && ( DotProduct( f->velDir, fNext->velDir ) > 0.999 ) ) {
					if( !droppedTrail ) {
