...
typedef enum { ZOOM_NONE, ZOOM_BINOC, ZOOM_SNIPER, ZOOM_SNOOPER, ZOOM_FG42SCOPE, ZOOM_MG42, ZOOM_MAX_ZOOMS } EZoom_t;
...
typedef enum { LEBS_NONE, LEBS_BLOOD, LEBS_ROCK, LEBS_WOOD, LEBS_BRASS, LEBS_BONE } leBounceSoundType_t; // fragment local entities can make sounds on impacts
...
typedef enum { W_PART_1, W_PART_2, W_PART_3, W_PART_4, W_PART_5, W_PART_6, W_PART_7, W_MAX_PARTS } barrelType_t;
...
void			CG_DrawHoldableSelect();

void			CG_OutOfAmmoChange();
void			CG_HoldableUsedupChange(); //----(SA)	added
...
void			CG_AddMarks();
void			CG_ImpactMark(
			   qhandle_t markShader, const vec3_t origin, const vec3_t dir, float orientation, float r, float g, float b, float a, qboolean alphaFade, float radius, qboolean temporary, int duration );
...
void		   CG_ParticleImpactSmokePuffExtended( qhandle_t pshader,
			  ...
			  int											 acc,
			  int											 maxroll,
			  float											 alpha ); // (SA) so I can add more parameters without screwing up the one that's there
...
void		   CG_InitFlameChunks();
void		   CG_AddFlameChunks();
...
typedef enum { SYSTEM_PRINT, CHAT_PRINT, TEAMCHAT_PRINT } q3print_t; // bk001201 - warning: useless keyword or type name in empty declaration
