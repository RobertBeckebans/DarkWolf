...
void CG_DrawFlagModel( float x, float y, float w, float h, int team )
{
	...
	CG_Draw3DModel( x, y, w, h, team == TEAM_RED ? cgs.media.redFlagModel : cgs.media.blueFlagModel, 0, origin, angles );
}
...
static void CG_DrawStatusBar()
{
	...
	*/
	...
	if( cent->currentState.weapon ) {
		...
		if( value > -1 ) {
			if( ( cg.predictedPlayerState.weaponstate == WEAPON_FIRING || cg.predictedPlayerState.weaponstate == WEAPON_FIRINGALT ) && cg.predictedPlayerState.weaponTime > 100 ) {
...
static float CG_DrawTeamOverlay( float y )
{
	...
	for( i = 0; i < numSortedTeamPlayers; i++ ) {
		...
		if( ci->infoValid && ci->team == cg.snap->ps.persistant[PERS_TEAM] ) {
			...
			CG_DrawPic( xx, y, TINYCHAR_WIDTH, TINYCHAR_HEIGHT, cg_weapons[ci->curWeapon].weaponIcon[0] );
			...
			for( j = 0; j < PW_NUM_POWERUPS; j++ ) {
				if( ci->powerups & ( 1 << j ) ) {
					...
					if( item != NULL ) { // JPW NERVE added for invulnerability powerup at beginning of map
						CG_DrawPic( xx, y, TINYCHAR_WIDTH, TINYCHAR_HEIGHT, sys->R_RegisterShader( item->icon ) );
						xx -= TINYCHAR_WIDTH;
					} // jpw
				}
			}
...
static float CG_DrawPowerups( float y )
{
	...
	for( i = 0; i < active; i++ ) {
		...
		CG_DrawPic( 640 - size, y + ICON_SIZE / 2 - size / 2, size, size, sys->R_RegisterShader( item->icon ) );
	}
...
==============
*/
static void CG_DrawWeapReticle()
{
	int	   weap;
...
static void CG_DrawCrosshair()
{
	...
	*/
	...
	sys->R_DrawStretchPic( x + cg.refdef.x + 0.5 * ( cg.refdef.width - w ), y + cg.refdef.y + 0.5 * ( cg.refdef.height - h ), w, h, 0, 0, 1, 1, hShader );
}
...
==============
*/
static void CG_DrawDynamiteStatus()
{
	float color[4];
	char* name;
	int	  timeleft;
...
static void CG_DrawCrosshairNames()
{
	...
	if( strlen( name ) ) {
		...
		} else if( ( cgs.clientinfo[cg.crosshairClientNum].team == TEAM_BLUE ) && ( cgs.clientinfo[cg.snap->ps.clientNum].team == TEAM_BLUE ) ) { // JPW NERVE -- so's we can't find snipers for free
...
// TTimo: unused
/*
static void CG_ActivateLimboMenu() {
...
==============
*/
static void CG_DrawFlashZoomTransition()
{
	vec4_t color;
	float  frac;
	int	   fadeTime;
...
=================
*/
static void CG_DrawFlashDamage()
{
	vec4_t col;
	float  redFlash;
...
=================
*/
static void CG_DrawFlashLightning()
{
...
==============
*/
static void CG_DrawFlashBlendBehindHUD()
{
	CG_DrawFlashZoomTransition();
}
...
static void CG_DrawObjectiveInfo()
{
	char*  start;
	int	   l;
	int	   x, y, w;
...
===============
*/
static void CG_DrawGameScreenFade()
{
	vec4_t col;
