...
int			weapBanksMultiPlayer[MAX_WEAP_BANKS_MP][MAX_WEAPS_IN_BANK_MP] = { { 0, 0, 0, 0, 0, 0, 0, 0 }, // empty bank '0'
...
static qboolean CG_ParseWeaponConfig( const char* filename, weaponInfo_t* wi )
{
	...
	for( i = 0; i < MAX_WP_ANIMATIONS; i++ ) {
		...
		if( newfmt ) {
			...
			token = COM_Parse(
				&text_p ); // barrel hide bits (so objects can be flagged to not be drawn during all sequences (a reloading hand that comes in from off screen for that one animation for example)
			if( !token ) {
				break;
			}
...
static void CG_RunWeapLerpFrame( clientInfo_t* ci, weaponInfo_t* wi, lerpFrame_t* lf, int newAnimation, float speedScale )
{
	...
	if( !lf->animation ) {
		...
		if( ( newAnimation & ~ANIM_TOGGLEBIT ) == WEAP_RAISE || ( newAnimation & ~ANIM_TOGGLEBIT ) == WEAP_ALTSWITCHFROM || ( newAnimation & ~ANIM_TOGGLEBIT ) == WEAP_ALTSWITCHTO ) {
...
#define TESLA_MAX_POINT_TESTS_PERFRAME 20
	...
	if( !cent->pe.teslaDamageApplyTime || cent->pe.teslaDamageApplyTime < cg.time - 200 ) {
		...
		VectorNormalize( vec );
		if( ( cent != &cg_entities[cg.snap->ps.clientNum] ) && ( cg.snap->ps.teamNum != playerTeam ) && ( Distance( tagPos, cg.snap->ps.origin ) < TESLA_LIGHTNING_MAX_DIST ) &&
		...
		if( cgs.localServer && cgs.gametype == GT_SINGLE_PLAYER ) {
			...
			for( ctrav = cg_entities, i = 0; i < cgs.maxclients && numEnemies < 16; ctrav++, i++ ) {
				...
				*/                                                                                                                              //
				if( ctrav->currentState.aiChar && ( ctrav != cent ) && ( ctrav->currentState.teamNum != playerTeam ) && !( ctrav->currentState.eFlags & EF_DEAD ) &&
	...
	for( i = 0; i < numPoints; i++ ) {
		...
		} else if( ( !cent->pe.teslaEndPointTimes[i] ) || ( cent->pe.teslaEndPointTimes[i] > cg.time ) || ( cent->pe.teslaEndPointTimes[i] < cg.time - TESLA_LIGHTNING_POINT_TIMEOUT ) ||
				   ( VectorDistance( tagPos, cent->pe.teslaEndPoints[i] ) > maxDist ) || ( DotProduct( viewDir, vec ) < 0.7 ) ) {
			// if (cent->currentState.groundEntityNum == ENTITYNUM_NONE)
			//	continue;	// must be on the ground

			// find a new spot
			for( j = 0; j < TESLA_MAX_POINT_TESTS; j++ ) {
				VectorSet( testPos, cg.refdef.fov_y * crandom() * 0.5, cg.refdef.fov_x * crandom() * 0.5, 0 );
...
void			CG_AddPlayerWeapon( refEntity_t* parent, playerState_t* ps, centity_t* cent )
{
	...
	if( weaponNum == WP_SNOOPERSCOPE || weaponNum == WP_SNIPERRIFLE || weaponNum == WP_FG42SCOPE || weapSelect == WP_SNOOPERSCOPE || weapSelect == WP_SNIPERRIFLE || weapSelect == WP_FG42SCOPE ) {
...
#define BARREL_SMOKE_TIME 1000
	...
	if( weaponNum == WP_GRENADE_LAUNCHER || weaponNum == WP_GRENADE_PINEAPPLE || weaponNum == WP_KNIFE || weaponNum == WP_DYNAMITE ) {
		return;
	}
	...
	if( ps || cg.renderingThirdPerson || !isPlayer ) {
		if( firing ) {
			...
			if( weapon->flashDlightColor[0] || weapon->flashDlightColor[1] || weapon->flashDlightColor[2] ) {
				sys->R_AddLightToScene( flash.origin, 200 + ( rand() & 31 ), weapon->flashDlightColor[0], weapon->flashDlightColor[1], weapon->flashDlightColor[2], 0 );
			}
		} else {
...
static qboolean CG_WeaponHasAmmo( int i )
{
	if( !( cg.predictedPlayerState.ammo[BG_FindAmmoForWeapon( ( weapon_t )i )] ) && !( cg.predictedPlayerState.ammoclip[BG_FindClipForWeapon( ( weapon_t )i )] ) ) {
		return qfalse;
	}
...
void CG_FireWeapon( centity_t* cent )
{
	...
	} else if( ent->weapon == WP_GRENADE_LAUNCHER || ent->weapon == WP_GRENADE_PINEAPPLE || ent->weapon == WP_DYNAMITE || ent->weapon == WP_GRENADE_SMOKE ) { // JPW NERVE
...
=================
*/
void CG_AddDirtBulletParticles( vec3_t origin, vec3_t dir, int speed, int duration, int count, float randScale, float width, float height, float alpha, char* shadername ) // JPW NERVE
{
	vec3_t velocity, pos;
	int	   i;
	...
	for( i = 0; i < count; i++ ) {
		...
		CG_ParticleDirtBulletDebris_Core( pos,
			velocity,
		...
		case WP_VENOM:
			...
			if( !clientNum ) {
				...
				if( surfFlags & SURF_SNOW ) {
					CG_AddDirtBulletParticles( origin,
						dir,
						120, // speed
						900, // duration
						3,	 // count
		...
		case WP_DYNAMITE:
			...
			if( cg_gameType.integer == GT_SINGLE_PLAYER ) { // JPW NERVE
				for( i = 0; i < 5; i++ ) {
					...
					sprVel[2] += rand() % 50;
					CG_ParticleExplosion( "blacksmokeanimb",
						sprOrg,
						sprVel,
		...
		*/
		...
		if( lastBloodSpat > cg.time || lastBloodSpat < cg.time - 500 ) {
			if( CG_CalcMuzzlePoint( sourceEntityNum, start ) ) {
				...
				if( trace.fraction < 1 ) {
					CG_ImpactMark(
						cgs.media.bloodDotShaders[rand() % 5], trace.endpos, trace.plane.normal, random() * 360, 1, 1, 1, 1, qtrue, 15 + random() * 20, qfalse, cg_bloodTime.integer * 1000 );
					...
					if( trace.fraction < 1 ) {
						CG_ImpactMark(
							cgs.media.bloodDotShaders[rand() % 5], trace.endpos, trace.plane.normal, random() * 360, 1, 1, 1, 1, qtrue, 15 + random() * 10, qfalse, cg_bloodTime.integer * 1000 );
						lastBloodSpat = cg.time;
					}
				}
			}
