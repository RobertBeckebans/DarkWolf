...
void CG_AttachedPartChange( centity_t* cent )
{
	...
	if( aiCharNum == AICHAR_PROTOSOLDIER || aiCharNum == AICHAR_SUPERSOLDIER || aiCharNum == AICHAR_HEINRICH ) {
...
static void CG_Missile( centity_t* cent )
{
	...
	if( weapon->missileDlight ) {
		sys->R_AddLightToScene( cent->lerpOrigin, weapon->missileDlight, weapon->missileDlightColor[0], weapon->missileDlightColor[1], weapon->missileDlightColor[2], 0 );
	}
...
static void CG_Corona( centity_t* cent )
{
	...
	if( !behind && !toofar ) {
		CG_Trace( &tr,
			...
			cent->lerpOrigin,
			-1,
			MASK_SOLID | CONTENTS_BODY ); // added blockage by players.  not sure how this is going to be since this is their bb, not their model (too much blockage)
