...
METHODDEF void pre_process_data(
		...
		*in_row_ctr += numrows;
		...
		if( prep->rows_to_go == 0 && *out_row_group_ctr < out_row_groups_avail ) {
			for( ci = 0, compptr = cinfo->comp_info; ci < cinfo->num_components; ci++, compptr++ ) {
				expand_bottom_edge(
					output_buf[ci], compptr->width_in_blocks * DCTSIZE, ( int )( *out_row_group_ctr * compptr->v_samp_factor ), ( int )( out_row_groups_avail * compptr->v_samp_factor ) );
			}
...
METHODDEF void pre_process_context(
	j_compress_ptr cinfo, JSAMPARRAY input_buf, JDIMENSION* in_row_ctr, JDIMENSION in_rows_avail, JSAMPIMAGE output_buf, JDIMENSION* out_row_group_ctr, JDIMENSION out_row_groups_avail )
{
	my_prep_ptr			 prep = ( my_prep_ptr )cinfo->prep;
	int					 numrows, ci;
...
LOCAL void create_context_buffer( j_compress_ptr cinfo )
{
	...
	for( ci = 0, compptr = cinfo->comp_info; ci < cinfo->num_components; ci++, compptr++ ) {
		 ...
		 */
		true_buffer = ( *cinfo->mem->alloc_sarray )( ( j_common_ptr )cinfo,
			JPOOL_IMAGE,
...
GLOBAL void jinit_c_prep_controller( j_compress_ptr cinfo, boolean need_full_buffer )
{
	...
	if( cinfo->downsample->need_context_rows ) {
		...
		for( ci = 0, compptr = cinfo->comp_info; ci < cinfo->num_components; ci++, compptr++ ) {
			prep->color_buf[ci] = ( *cinfo->mem->alloc_sarray )( ( j_common_ptr )cinfo,
				JPOOL_IMAGE,
				( JDIMENSION )( ( ( long )compptr->width_in_blocks * DCTSIZE * cinfo->max_h_samp_factor ) / compptr->h_samp_factor ),
