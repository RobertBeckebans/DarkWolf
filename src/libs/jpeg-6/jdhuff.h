...
EXTERN boolean jpeg_fill_bit_buffer JPP( ( bitread_working_state * state, register bit_buf_type get_buffer, register int bits_left, int nbits ) );
...
#define HUFF_DECODE( result, state, htbl, failaction, slowlabel )                                              \
	{                                                                                                          \
		...
		if( ( nb = htbl->look_nbits[look] ) != 0 ) {                                                           \
			...
			nb = HUFF_LOOKAHEAD + 1;                                                                           \
		slowlabel:                                                                                             \
			if( ( result = jpeg_huff_decode( &state, get_buffer, bits_left, htbl, nb ) ) < 0 ) { failaction; } \
			get_buffer = state.get_buffer;                                                                     \
			bits_left  = state.bits_left;                                                                      \
		}                                                                                                      \
	}
...
EXTERN int jpeg_huff_decode JPP( ( bitread_working_state * state, register bit_buf_type get_buffer, register int bits_left, d_derived_tbl* htbl, int min_bits ) );
