...
#define GLR_CHECK( x )                                                                                  \
	do {                                                                                                \
		HRESULT _hr = ( x );                                                                            \
		if( FAILED( _hr ) ) {                                                                           \
			glRaytracingFatal( "HRESULT 0x%08X failed at %s:%d", ( unsigned )_hr, __FILE__, __LINE__ ); \
			return 0;                                                                                   \
		}                                                                                               \
	} while( 0 )
...
static glRaytracingBuffer_t glRaytracingCreateBuffer( ID3D12Device* device, UINT64 size, D3D12_HEAP_TYPE heapType, D3D12_RESOURCE_STATES initialState, D3D12_RESOURCE_FLAGS flags )
{
	...
	HRESULT hr = device->CreateCommittedResource( &hp, D3D12_HEAP_FLAG_NONE, &rd, initialState, nullptr, IID_PPV_ARGS( &out.resource ) );
...
static void glRaytracingTransition( ID3D12GraphicsCommandList* cmd, ID3D12Resource* res, D3D12_RESOURCE_STATES before, D3D12_RESOURCE_STATES after )
{
	if( !res || before == after )
		return;
...
static int glRaytracingInitCmdContext( void )
{
	...
	GLR_CHECK( g_glRaytracingCmd.device->CreateCommandAllocator( D3D12_COMMAND_LIST_TYPE_DIRECT, IID_PPV_ARGS( &g_glRaytracingCmd.cmdAlloc ) ) );

	GLR_CHECK( g_glRaytracingCmd.device->CreateCommandList( 0, D3D12_COMMAND_LIST_TYPE_DIRECT, g_glRaytracingCmd.cmdAlloc.Get(), nullptr, IID_PPV_ARGS( &g_glRaytracingCmd.cmdList ) ) );

	GLR_CHECK( g_glRaytracingCmd.cmdList->Close() );

	GLR_CHECK( g_glRaytracingCmd.device->CreateCommandAllocator( D3D12_COMMAND_LIST_TYPE_DIRECT, IID_PPV_ARGS( &g_glRaytracingCmd.blasCmdAlloc ) ) );

	GLR_CHECK( g_glRaytracingCmd.device->CreateCommandList( 0, D3D12_COMMAND_LIST_TYPE_DIRECT, g_glRaytracingCmd.blasCmdAlloc.Get(), nullptr, IID_PPV_ARGS( &g_glRaytracingCmd.blasCmdList ) ) );

	GLR_CHECK( g_glRaytracingCmd.blasCmdList->Close() );

	GLR_CHECK( g_glRaytracingCmd.device->CreateCommandAllocator( D3D12_COMMAND_LIST_TYPE_DIRECT, IID_PPV_ARGS( &g_glRaytracingCmd.tlasCmdAlloc ) ) );

	GLR_CHECK( g_glRaytracingCmd.device->CreateCommandList( 0, D3D12_COMMAND_LIST_TYPE_DIRECT, g_glRaytracingCmd.tlasCmdAlloc.Get(), nullptr, IID_PPV_ARGS( &g_glRaytracingCmd.tlasCmdList ) ) );

	GLR_CHECK( g_glRaytracingCmd.tlasCmdList->Close() );

	GLR_CHECK( g_glRaytracingCmd.device->CreateFence( 0, D3D12_FENCE_FLAG_NONE, IID_PPV_ARGS( &g_glRaytracingCmd.fence ) ) );
...
static int glRaytracingEnsureTLASBuffers( const D3D12_BUILD_RAYTRACING_ACCELERATION_STRUCTURE_INPUTS* inputs )
{
	...
	const UINT64 requiredScratch = glRaytracingAlignUp( prebuild.ScratchDataSizeInBytes, D3D12_RAYTRACING_ACCELERATION_STRUCTURE_BYTE_ALIGNMENT );

	const UINT64 requiredResult = glRaytracingAlignUp( prebuild.ResultDataMaxSizeInBytes, D3D12_RAYTRACING_ACCELERATION_STRUCTURE_BYTE_ALIGNMENT );

	if( !g_glRaytracingScene.tlasScratch.resource || g_glRaytracingScene.tlasScratchSize < requiredScratch ) {
		g_glRaytracingScene.tlasScratch.resource.Reset();
		g_glRaytracingScene.tlasScratch =
			glRaytracingCreateBuffer( g_glRaytracingCmd.device.Get(), requiredScratch, D3D12_HEAP_TYPE_DEFAULT, D3D12_RESOURCE_STATE_COMMON, D3D12_RESOURCE_FLAG_ALLOW_UNORDERED_ACCESS );
	...
	for( int i = 0; i < 2; ++i ) {
		if( !g_glRaytracingScene.tlasResult[i].resource || g_glRaytracingScene.tlasResultSize < requiredResult ) {
			g_glRaytracingScene.tlasResult[i].resource.Reset();
			g_glRaytracingScene.tlasResult[i] = glRaytracingCreateBuffer(
				g_glRaytracingCmd.device.Get(), requiredResult, D3D12_HEAP_TYPE_DEFAULT, D3D12_RESOURCE_STATE_RAYTRACING_ACCELERATION_STRUCTURE, D3D12_RESOURCE_FLAG_ALLOW_UNORDERED_ACCESS );
...
static inline void glRaytracingBuildInstanceDesc( D3D12_RAYTRACING_INSTANCE_DESC* outDesc, const glRaytracingInstanceRecord_t& inst, D3D12_GPU_VIRTUAL_ADDRESS blasGpuVA )
{
...
static int glRaytracingResolveInstanceDesc( glRaytracingInstanceRecord_t* inst, D3D12_RAYTRACING_INSTANCE_DESC* outDesc, D3D12_GPU_VIRTUAL_ADDRESS* outBlasGpuVA )
{
	if( !inst || !inst->alive )
		return 0;
...
static int glRaytracingUploadCachedInstanceDescs( void )
{
	...
	const UINT64 instBytes	 = glRaytracingAlignUp( ( UINT64 )activeCount * ( UINT64 )sizeof( D3D12_RAYTRACING_INSTANCE_DESC ), D3D12_RAYTRACING_INSTANCE_DESCS_BYTE_ALIGNMENT );
...
static int glRaytracingBuildDirtyMeshesInternal( void )
{
	...
	for( size_t i = 0; i < dirtyMeshes.size(); ++i ) {
		...
		info.inputs.Flags = mesh->descCpu.allowUpdate ? ( D3D12_RAYTRACING_ACCELERATION_STRUCTURE_BUILD_FLAG_PREFER_FAST_TRACE | D3D12_RAYTRACING_ACCELERATION_STRUCTURE_BUILD_FLAG_ALLOW_UPDATE ) :
...
static int glRaytracingUploadMeshBuffers( glRaytracingMeshRecord_t* mesh )
{
	...
	mesh->indexBuffer = glRaytracingCreateBuffer( g_glRaytracingCmd.device.Get(), ibBytes, D3D12_HEAP_TYPE_UPLOAD, D3D12_RESOURCE_STATE_GENERIC_READ, D3D12_RESOURCE_FLAG_NONE );
...
static int glRaytracingEnsureSceneUploadBuffer( UINT64 requiredBytes )
{
	...
	requiredBytes = glRaytracingAlignUp( requiredBytes, D3D12_RAYTRACING_INSTANCE_DESCS_BYTE_ALIGNMENT );

	if( upload->buffer.resource && upload->capacityBytes >= requiredBytes && upload->mapped ) {
		return 1;
	}
	...
	upload->buffer = glRaytracingCreateBuffer( g_glRaytracingCmd.device.Get(), requiredBytes, D3D12_HEAP_TYPE_UPLOAD, D3D12_RESOURCE_STATE_GENERIC_READ, D3D12_RESOURCE_FLAG_NONE );
...
static ComPtr<IDxcBlob>			   glRaytracingLightingCompileLibrary( const char* src )
{
	...
	const wchar_t*	   args[] = { L"-T", L"lib_6_3", L"-Zi", L"-Qembed_debug", L"-O3", L"-all_resources_bound" };
...
static int glRaytracingLightingCreateDescriptorHeap( void )
{
	...
	g_glRaytracingLighting.descriptorStride = g_glRaytracingCmd.device->GetDescriptorHandleIncrementSize( D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV );
...
{
	{
		...
		GLR_CHECK( g_glRaytracingCmd.device->CreateRootSignature( 0, sig->GetBufferPointer(), sig->GetBufferSize(), IID_PPV_ARGS( &g_glRaytracingLighting.globalRootSig ) ) );
	}

	{
		...
		ComPtr<ID3DBlob> err;
		GLR_CHECK( D3D12SerializeRootSignature( &rsd, D3D_ROOT_SIGNATURE_VERSION_1, &sig, &err ) );
		GLR_CHECK( g_glRaytracingCmd.device->CreateRootSignature( 0, sig->GetBufferPointer(), sig->GetBufferSize(), IID_PPV_ARGS( &g_glRaytracingLighting.localRootSig ) ) );
	}
...
static int glRaytracingLightingCreateBuffers( void )
{
	g_glRaytracingLighting.constantBuffer = glRaytracingCreateBuffer(
		g_glRaytracingCmd.device.Get(), glRaytracingAlignUp( sizeof( glRaytracingLightingConstants_t ), 256 ), D3D12_HEAP_TYPE_UPLOAD, D3D12_RESOURCE_STATE_GENERIC_READ, D3D12_RESOURCE_FLAG_NONE );

	g_glRaytracingLighting.lightBuffer = glRaytracingCreateBuffer(
		g_glRaytracingCmd.device.Get(), sizeof( glRaytracingLight_t ) * GL_RAYTRACING_MAX_LIGHTS, D3D12_HEAP_TYPE_UPLOAD, D3D12_RESOURCE_STATE_GENERIC_READ, D3D12_RESOURCE_FLAG_NONE );
...
{
	glRaytracingMapCopy( g_glRaytracingLighting.constantBuffer.resource.Get(), &g_glRaytracingLighting.constants, sizeof( g_glRaytracingLighting.constants ) );
}
...
static void glRaytracingLightingUpdateLights( void )
{
	...
	glRaytracingMapCopy( g_glRaytracingLighting.lightBuffer.resource.Get(), g_glRaytracingLighting.cpuLights.data(), g_glRaytracingLighting.cpuLights.size() * sizeof( glRaytracingLight_t ) );
}
...
static void glRaytracingLightingCreatePersistentLightSRV( void )
{
	...
	g_glRaytracingCmd.device->CreateShaderResourceView( g_glRaytracingLighting.lightBuffer.resource.Get(), &srv, glRaytracingOffsetCpu( base, g_glRaytracingLighting.descriptorStride, 0 ) );
}
...
static int glRaytracingLightingCreateShaderTables( void )
{
	...
	g_glRaytracingLighting.raygenTable = glRaytracingCreateBuffer( g_glRaytracingCmd.device.Get(), recordSize, D3D12_HEAP_TYPE_UPLOAD, D3D12_RESOURCE_STATE_GENERIC_READ, D3D12_RESOURCE_FLAG_NONE );

	g_glRaytracingLighting.missTable = glRaytracingCreateBuffer( g_glRaytracingCmd.device.Get(), recordSize, D3D12_HEAP_TYPE_UPLOAD, D3D12_RESOURCE_STATE_GENERIC_READ, D3D12_RESOURCE_FLAG_NONE );

	g_glRaytracingLighting.hitTable = glRaytracingCreateBuffer( g_glRaytracingCmd.device.Get(), recordSize, D3D12_HEAP_TYPE_UPLOAD, D3D12_RESOURCE_STATE_GENERIC_READ, D3D12_RESOURCE_FLAG_NONE );
...
static void glRaytracingLightingCreatePerPassDescriptors( const glRaytracingLightingPassDesc_t* pass )
{
	...
	g_glRaytracingCmd.device->CreateShaderResourceView( pass->albedoTexture, &albedoSrv, glRaytracingOffsetCpu( base, g_glRaytracingLighting.descriptorStride, 1 ) );
	...
	g_glRaytracingCmd.device->CreateShaderResourceView( pass->depthTexture, &depthSrv, glRaytracingOffsetCpu( base, g_glRaytracingLighting.descriptorStride, 2 ) );
	...
	g_glRaytracingCmd.device->CreateShaderResourceView( pass->normalTexture, &normalSrv, glRaytracingOffsetCpu( base, g_glRaytracingLighting.descriptorStride, 3 ) );
	...
	g_glRaytracingCmd.device->CreateShaderResourceView( pass->positionTexture, &positionSrv, glRaytracingOffsetCpu( base, g_glRaytracingLighting.descriptorStride, 4 ) );
	...
	g_glRaytracingCmd.device->CreateShaderResourceView( nullptr, &tlasSrv, glRaytracingOffsetCpu( base, g_glRaytracingLighting.descriptorStride, 5 ) );
	...
	g_glRaytracingCmd.device->CreateUnorderedAccessView( pass->outputTexture, nullptr, &outputUav, glRaytracingOffsetCpu( base, g_glRaytracingLighting.descriptorStride, 6 ) );
}
...
extern "C" bool glRaytracingLightingExecute( const glRaytracingLightingPassDesc_t* pass )
{
	...
	if( !pass->albedoTexture || !pass->depthTexture || !pass->normalTexture || !pass->positionTexture || !pass->outputTexture || !pass->topLevelAS ) {
		return false;
	}
	...
	g_glRaytracingLighting.constants.lightCount	   = ( uint32_t )glRaytracingClamp<size_t>( g_glRaytracingLighting.cpuLights.size(), 0, GL_RAYTRACING_MAX_LIGHTS );
	...
	glRaytracingTransition( g_glRaytracingCmd.cmdList.Get(), pass->outputTexture, D3D12_RESOURCE_STATE_PIXEL_SHADER_RESOURCE, D3D12_RESOURCE_STATE_UNORDERED_ACCESS );
	...
	g_glRaytracingCmd.cmdList->SetComputeRootDescriptorTable( 0, glRaytracingOffsetGpu( gpuBase, g_glRaytracingLighting.descriptorStride, 0 ) );
	g_glRaytracingCmd.cmdList->SetComputeRootDescriptorTable( 1, glRaytracingOffsetGpu( gpuBase, g_glRaytracingLighting.descriptorStride, 6 ) );
	...
	const UINT				 shaderRecordSize = ( UINT )glRaytracingAlignUp( D3D12_SHADER_IDENTIFIER_SIZE_IN_BYTES, D3D12_RAYTRACING_SHADER_RECORD_BYTE_ALIGNMENT );
	...
	if( backBuffer ) {
		glRaytracingTransition( g_glRaytracingCmd.cmdList.Get(), pass->outputTexture, D3D12_RESOURCE_STATE_UNORDERED_ACCESS, D3D12_RESOURCE_STATE_COPY_SOURCE );

		glRaytracingTransition( g_glRaytracingCmd.cmdList.Get(), backBuffer, D3D12_RESOURCE_STATE_PRESENT, D3D12_RESOURCE_STATE_COPY_DEST );

		g_glRaytracingCmd.cmdList->CopyResource( backBuffer, pass->outputTexture );

		glRaytracingTransition( g_glRaytracingCmd.cmdList.Get(), backBuffer, D3D12_RESOURCE_STATE_COPY_DEST, D3D12_RESOURCE_STATE_PRESENT );

		glRaytracingTransition( g_glRaytracingCmd.cmdList.Get(), pass->outputTexture, D3D12_RESOURCE_STATE_COPY_SOURCE, D3D12_RESOURCE_STATE_PIXEL_SHADER_RESOURCE );
	} else {
		glRaytracingTransition( g_glRaytracingCmd.cmdList.Get(), pass->outputTexture, D3D12_RESOURCE_STATE_UNORDERED_ACCESS, D3D12_RESOURCE_STATE_PIXEL_SHADER_RESOURCE );
	}
...
extern "C" glRaytracingLight_t glRaytracingLightingMakePointLight( float px, float py, float pz, float radius, float r, float g, float b, float intensity )
{
...
extern "C" glRaytracingLight_t glRaytracingLightingMakeRectLight( float px,
	float																py,
	float																pz,
	float																nx,
	float																ny,
	...
	uint32_t															twoSided )
{
	...
	if( ( nx == 0.0f && ny == 0.0f && nz == 0.0f ) && !( ( ux == 0.0f && uy == 0.0f && uz == 0.0f ) || ( vx == 0.0f && vy == 0.0f && vz == 0.0f ) ) ) {
