...
static D3D12_GRAPHICS_PIPELINE_STATE_DESC BuildPSODesc( PipelineMode mode, ID3DBlob* vs, ID3DBlob* ps, GLenum srcBlendGL, GLenum dstBlendGL, bool depthTest, bool depthWrite, GLenum depthFuncGL )
{
	...
	d.DepthStencilState.DepthWriteMask = ( depthTest && depthWrite ) ? D3D12_DEPTH_WRITE_MASK_ALL : D3D12_DEPTH_WRITE_MASK_ZERO;
...
static void QD3D12_CreatePSOs()
{
	...
	auto descOpaqueTex = BuildPSODesc( PIPE_OPAQUE_TEX,
		g_gl.vsMainBlob.Get(),
		g_gl.psMainBlob.Get(),
		GL_ONE,
		GL_ZERO,
	...
	auto descAlphaTex = BuildPSODesc( PIPE_ALPHA_TEST_TEX,
		g_gl.vsMainBlob.Get(),
	...
	auto descOpaqueUntex = BuildPSODesc( PIPE_OPAQUE_UNTEX,
		g_gl.vsMainBlob.Get(),
...
static uint64_t MakePSOKey( PipelineMode mode, GLenum src, GLenum dst, bool depthTest, bool depthWrite, GLenum depthFunc )
{
	return ( uint64_t( uint32_t( mode ) ) << 56ull ) | ( uint64_t( uint32_t( src ) & 0xFF ) << 48ull ) | ( uint64_t( uint32_t( dst ) & 0xFF ) << 40ull ) | ( uint64_t( depthTest ? 1 : 0 ) << 39ull ) |
		   ( uint64_t( depthWrite ? 1 : 0 ) << 38ull ) | ( uint64_t( uint32_t( depthFunc ) & 0xFFFF ) << 16ull );
}

static std::unordered_map<uint64_t, ComPtr<ID3D12PipelineState>> g_psoCache;

static ID3D12PipelineState*										 QD3D12_GetPSO( PipelineMode mode, GLenum srcBlendGL, GLenum dstBlendGL, bool depthTest, bool depthWrite, GLenum depthFuncGL )
{
	const uint64_t key = MakePSOKey( mode, srcBlendGL, dstBlendGL, depthTest, depthWrite, depthFuncGL );
	...
	auto						desc = BuildPSODesc( mode, g_gl.vsMainBlob.Get(), psBlob, srcBlendGL, dstBlendGL, depthTest, depthWrite, depthFuncGL );
...
static void QD3D12_FlushQueuedBatches()
{
	...
	for( size_t i = 0; i < g_gl.queuedBatches.size(); ++i ) {
		...
		switch( batch.key.pipeline ) {
			case PIPE_BLEND_TEX:
			case PIPE_BLEND_UNTEX:
				pso = QD3D12_GetPSO( batch.key.pipeline, batch.key.blendSrc, batch.key.blendDst, batch.key.depthTest, batch.key.depthWrite, batch.key.depthFunc );
				break;
