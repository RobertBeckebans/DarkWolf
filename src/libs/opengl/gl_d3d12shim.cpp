...
#define QD3D12_CHECK( x )                                                                          \
	do {                                                                                           \
		HRESULT _hr = ( x );                                                                       \
		if( FAILED( _hr ) ) {                                                                      \
			QD3D12_Fatal( "HRESULT failed 0x%08X at %s:%d", ( unsigned )_hr, __FILE__, __LINE__ ); \
		}                                                                                          \
	} while( 0 )
...
enum PipelineMode { PIPE_OPAQUE_TEX = 0, PIPE_ALPHA_TEST_TEX, PIPE_BLEND_TEX, PIPE_OPAQUE_UNTEX, PIPE_BLEND_UNTEX, PIPE_COUNT };
...
enum TexEnvModeShader { TEXENV_MODULATE = 0, TEXENV_REPLACE = 1, TEXENV_DECAL = 2, TEXENV_BLEND = 3, TEXENV_ADD = 4 };

static float MapTexEnvMode( GLenum mode )
{
	switch( mode ) {
		case GL_REPLACE:
			return ( float )TEXENV_REPLACE;
		case GL_BLEND:
			return ( float )TEXENV_BLEND;
#ifdef GL_ADD
		case GL_ADD:
			return ( float )TEXENV_ADD;
#endif
		case GL_MODULATE:
		default:
			return ( float )TEXENV_MODULATE;
	}
}
...
{
	return ViewportEquals( a.viewport, b.viewport ) && RectEquals( a.scissor, b.scissor ) && a.pipeline == b.pipeline && a.topology == b.topology && a.tex0SrvIndex == b.tex0SrvIndex &&
		   a.tex1SrvIndex == b.tex1SrvIndex && a.alphaRef == b.alphaRef && a.useTex0 == b.useTex0 && a.useTex1 == b.useTex1 && a.tex1IsLightmap == b.tex1IsLightmap && a.texEnvMode0 == b.texEnvMode0 &&
		   a.texEnvMode1 == b.texEnvMode1 && a.blendSrc == b.blendSrc && a.blendDst == b.blendDst && a.depthTest == b.depthTest && a.depthWrite == b.depthWrite && a.depthFunc == b.depthFunc &&
		   a.geometryFlag == b.geometryFlag && memcmp( a.mvp.m, b.mvp.m, sizeof( a.mvp.m ) ) == 0 && memcmp( a.modelMatrix.m, b.modelMatrix.m, sizeof( a.modelMatrix.m ) ) == 0;
}
...
static inline float QD3D12_ReadScalarFast( const uint8_t* p, GLenum type )
{
	switch( type ) {
		...
		case GL_INT:
			return ( float )( *( const GLint* )p );
		case GL_UNSIGNED_INT:
			return ( float )( *( const GLuint* )p );
		case GL_SHORT:
			return ( float )( *( const GLshort* )p );
		case GL_UNSIGNED_SHORT:
			return ( float )( *( const GLushort* )p );
		case GL_BYTE:
			return ( float )( *( const GLbyte* )p );
		case GL_UNSIGNED_BYTE:
			return ( float )( *( const GLubyte* )p );
		default:
			return 0.0f;
	}
}
...
static void QD3D12_FetchArrayVertex( GLint idx, GLVertex& out )
{
	...
	if( va.enabled && va.ptr ) {
		...
		switch( va.type ) {
			...
			default:
				if( va.size > 0 )
					out.px = QD3D12_ReadScalarFast( p + 0 * typeSize, va.type );
				if( va.size > 1 )
					out.py = QD3D12_ReadScalarFast( p + 1 * typeSize, va.type );
				if( va.size > 2 )
					out.pz = QD3D12_ReadScalarFast( p + 2 * typeSize, va.type );
				break;
		}
	}
	...
	if( ca.enabled && ca.ptr ) {
		...
		if( ca.type == GL_UNSIGNED_BYTE ) {
			...
			if( ca.size > 0 )
				out.r = QD3D12_ReadScalarFast( p + 0 * typeSize, ca.type );
			if( ca.size > 1 )
				out.g = QD3D12_ReadScalarFast( p + 1 * typeSize, ca.type );
			if( ca.size > 2 )
				out.b = QD3D12_ReadScalarFast( p + 2 * typeSize, ca.type );
			if( ca.size > 3 )
				out.a = QD3D12_ReadScalarFast( p + 3 * typeSize, ca.type );
		}
	}
	...
	//
	{
		...
		if( tc.enabled && tc.ptr ) {
			...
			switch( tc.type ) {
				...
				default:
					if( tc.size > 0 )
						out.u0 = QD3D12_ReadScalarFast( p + 0 * typeSize, tc.type );
					if( tc.size > 1 )
						out.v0 = QD3D12_ReadScalarFast( p + 1 * typeSize, tc.type );
					break;
			}
		}
	}
	...
	//
	{
		...
		if( tc.enabled && tc.ptr ) {
			...
			switch( tc.type ) {
				...
				default:
					if( tc.size > 0 )
						out.u1 = QD3D12_ReadScalarFast( p + 0 * typeSize, tc.type );
					if( tc.size > 1 )
						out.v1 = QD3D12_ReadScalarFast( p + 1 * typeSize, tc.type );
					break;
			}
		}
	}
...
static UploadAlloc QD3D12_AllocUpload( UINT bytes, UINT alignment )
{
	...
	if( alignedOffset + bytes > g_gl.upload.size ) {
		QD3D12_Fatal( "Per-frame upload buffer overflow: need %u bytes, aligned offset %u, size %u", bytes, alignedOffset, g_gl.upload.size );
	}
...
static void QD3D12_CreateOcclusionQueryObjects()
{
	...
	QD3D12_CHECK( g_gl.device->CreateCommittedResource( &hp, D3D12_HEAP_FLAG_NONE, &rd, D3D12_RESOURCE_STATE_COPY_DEST, nullptr, IID_PPV_ARGS( &g_gl.occlusionReadback ) ) );
...
static void QD3D12_CreateSwapChain()
{
	...
	QD3D12_CHECK( g_gl.factory->CreateSwapChainForHwnd( g_gl.queue.Get(), g_gl.hwnd, &sd, nullptr, nullptr, &sc1 ) );
...
static void QD3D12_CreateRTVs()
{
	...
	for( UINT i = 0; i < QD3D12_FrameCount; ++i ) {
		...
		QD3D12_CHECK( g_gl.device->CreateCommittedResource( &hp, D3D12_HEAP_FLAG_NONE, &rd, D3D12_RESOURCE_STATE_RENDER_TARGET, &clear, IID_PPV_ARGS( &g_gl.normalBuffers[i] ) ) );
	...
	for( UINT i = 0; i < QD3D12_FrameCount; ++i ) {
		...
		QD3D12_CHECK( g_gl.device->CreateCommittedResource( &hp, D3D12_HEAP_FLAG_NONE, &rd, D3D12_RESOURCE_STATE_RENDER_TARGET, &clear, IID_PPV_ARGS( &g_gl.positionBuffers[i] ) ) );
	...
	for( UINT i = 0; i < QD3D12_FrameCount; ++i ) {
		...
		g_gl.device->CreateShaderResourceView( g_gl.normalBuffers[i].Get(), &sd, g_gl.normalSrvCpu[i] );
	}
	...
	for( UINT i = 0; i < QD3D12_FrameCount; ++i ) {
		...
		g_gl.device->CreateShaderResourceView( g_gl.positionBuffers[i].Get(), &sd, g_gl.positionSrvCpu[i] );
	}
}
...
static void QD3D12_CreateDSV()
{
	...
	QD3D12_CHECK( g_gl.device->CreateCommittedResource( &hp, D3D12_HEAP_FLAG_NONE, &rd, D3D12_RESOURCE_STATE_DEPTH_WRITE, &clear, IID_PPV_ARGS( &g_gl.depthBuffer ) ) );
...
static void QD3D12_CreateUploadRing()
{
	...
	for( UINT i = 0; i < QD3D12_FrameCount; ++i ) {
		QD3D12_CHECK( g_gl.device->CreateCommittedResource( &hp, D3D12_HEAP_FLAG_NONE, &rd, D3D12_RESOURCE_STATE_GENERIC_READ, nullptr, IID_PPV_ARGS( &g_gl.upload.resource[i] ) ) );

		g_gl.upload.gpuBase[i] = g_gl.upload.resource[i]->GetGPUVirtualAddress();

		QD3D12_CHECK( g_gl.upload.resource[i]->Map( 0, nullptr, reinterpret_cast<void**>( &g_gl.upload.cpuBase[i] ) ) );
	}
}
...
static ComPtr<ID3DBlob> CompileShaderVariant( const char* entry, const char* target )
{
	...
	ComPtr<ID3DBlob> err;
	HRESULT			 hr = D3DCompile( kQuakeWrapperHLSL, strlen( kQuakeWrapperHLSL ), nullptr, nullptr, nullptr, entry, target, flags, 0, &blob, &err );
	if( FAILED( hr ) )
...
static D3D12_GRAPHICS_PIPELINE_STATE_DESC BuildPSODesc(
	PipelineMode mode, ID3DBlob* vs, ID3DBlob* ps, GLenum srcBlendGL, GLenum dstBlendGL, bool depthTest, bool depthWrite, GLenum depthFuncGL, D3D12_PRIMITIVE_TOPOLOGY_TYPE topoType )
{
...
static uint64_t MakePSOKey( PipelineMode mode, GLenum src, GLenum dst, bool depthTest, bool depthWrite, GLenum depthFunc, D3D12_PRIMITIVE_TOPOLOGY_TYPE topoType )
{
	return ( uint64_t( uint32_t( mode ) ) << 56ull ) | ( uint64_t( uint32_t( src ) & 0xFF ) << 48ull ) | ( uint64_t( uint32_t( dst ) & 0xFF ) << 40ull ) | ( uint64_t( depthTest ? 1 : 0 ) << 39ull ) |
		   ( uint64_t( depthWrite ? 1 : 0 ) << 38ull ) | ( uint64_t( uint32_t( topoType ) & 0x3 ) << 36ull ) | ( uint64_t( uint32_t( depthFunc ) & 0xFFFF ) << 16ull );
}

static std::unordered_map<uint64_t, ComPtr<ID3D12PipelineState>> g_psoCache;

static ID3D12PipelineState* QD3D12_GetPSO( PipelineMode mode, GLenum srcBlendGL, GLenum dstBlendGL, bool depthTest, bool depthWrite, GLenum depthFuncGL, D3D12_PRIMITIVE_TOPOLOGY_TYPE topoType )
{
	const uint64_t key = MakePSOKey( mode, srcBlendGL, dstBlendGL, depthTest, depthWrite, depthFuncGL, topoType );
	...
	auto						desc = BuildPSODesc( mode, g_gl.vsMainBlob.Get(), psBlob, srcBlendGL, dstBlendGL, depthTest, depthWrite, depthFuncGL, topoType );
...
static void QD3D12_CreateWhiteTexture()
{
	...
	QD3D12_CHECK( g_gl.device->CreateCommittedResource( &hpDef, D3D12_HEAP_FLAG_NONE, &rd, D3D12_RESOURCE_STATE_COPY_DEST, nullptr, IID_PPV_ARGS( &g_gl.whiteTexture.texture ) ) );
...
void QD3D12_BeginFrame()
{
	...
	D3D12_CPU_DESCRIPTOR_HANDLE rtvs[3] = { CurrentRTV(), CurrentNormalRTV(), CurrentPositionRTV() };
...
static void EnsureTextureResource( TextureResource& tex )
{
	...
	if( !tex.texture ) {
		...
		if( ( int )desc.Width != tex.width || ( int )desc.Height != tex.height || desc.Format != dxgiFormat ) {
	...
	QD3D12_CHECK( g_gl.device->CreateCommittedResource( &hp, D3D12_HEAP_FLAG_NONE, &rd, D3D12_RESOURCE_STATE_COPY_DEST, nullptr, IID_PPV_ARGS( &tex.texture ) ) );
...
static TextureResource* QD3D12_EnsureLightingTexture( int width, int height )
{
	...
	const bool needsCreate = !g_lightingTexture->texture || g_lightingTexture->width != ( int )width || g_lightingTexture->height != ( int )height ||
	...
	QD3D12_CHECK( g_gl.device->CreateCommittedResource( &hp, D3D12_HEAP_FLAG_NONE, &rd, D3D12_RESOURCE_STATE_UNORDERED_ACCESS, nullptr, IID_PPV_ARGS( &g_lightingTexture->texture ) ) );
	...
	g_gl.device->CreateShaderResourceView( g_lightingTexture->texture.Get(), &srv, g_lightingTexture->srvCpu );
...
static void ConvertToRGBA8( const TextureResource& tex, std::vector<uint8_t>& outRGBA )
{
	...
	if( tex.format == GL_RGB ) {
		...
		while( i + 4 <= pixelCount && srcByteOffset + 16 <= srcBytes ) {
			...
			// lane2 = [r2 g2 b2 x]
			// lane3 = [r3 g3 b3 x]
			const __m128i shuffled = _mm_shuffle_epi8( in, _mm_setr_epi8( 0, 1, 2, char( 0x80 ), 3, 4, 5, char( 0x80 ), 6, 7, 8, char( 0x80 ), 9, 10, 11, char( 0x80 ) ) );
	...
	if( tex.sysmem.empty() ) {
		...
		for( int y = 0; y < tex.height; ++y ) {
			...
			while( x + 4 <= count && srcByteOffset + 16 <= srcBytes ) {
				...
				__m128i shuffled = _mm_shuffle_epi8( in, _mm_setr_epi8( 0, 1, 2, char( 0x80 ), 3, 4, 5, char( 0x80 ), 6, 7, 8, char( 0x80 ), 9, 10, 11, char( 0x80 ) ) );
...
static void QD3D12_EmitQueryMarkers( size_t beginIdx, size_t endIdx )
{
	for( size_t i = beginIdx; i < endIdx; ++i ) {
		...
		if( m.type == QueryMarker::Begin ) {
			...
			g_gl.cmdList->ResolveQueryData( g_gl.occlusionQueryHeap.Get(), D3D12_QUERY_TYPE_OCCLUSION, q.heapIndex, 1, g_gl.occlusionReadback.Get(), sizeof( UINT64 ) * q.heapIndex );
...
static void QD3D12_FlushQueuedBatches()
{
	...
	for( size_t i = 0; i < g_gl.queuedBatches.size(); ++i ) {
		...
		const D3D12_PRIMITIVE_TOPOLOGY_TYPE topoType = GetTopologyTypeFromTopology( batch.key.topology );
		...
		switch( batch.key.pipeline ) {
			case PIPE_BLEND_TEX:
			case PIPE_BLEND_UNTEX:
				pso = QD3D12_GetPSO( batch.key.pipeline, batch.key.blendSrc, batch.key.blendDst, batch.key.depthTest, batch.key.depthWrite, batch.key.depthFunc, topoType );
				break;
...
extern "C" void APIENTRY glClear( GLbitfield mask )
{
	...
	if( mask & GL_COLOR_BUFFER_BIT ) {
		const float cc[4] = { g_gl.clearColor[0], g_gl.clearColor[1], g_gl.clearColor[2], g_gl.clearColor[3] };
	...
	if( mask & GL_DEPTH_BUFFER_BIT ) {
		g_gl.cmdList->ClearDepthStencilView( dsv, D3D12_CLEAR_FLAG_DEPTH, ( FLOAT )ClampValue<GLclampd>( g_gl.clearDepthValue, 0.0, 1.0 ), 0, 1, &clearRect );
	}

	if( mask & GL_STENCIL_BUFFER_BIT ) {
		g_gl.cmdList->ClearDepthStencilView(
			dsv, D3D12_CLEAR_FLAG_STENCIL, ( FLOAT )ClampValue<GLclampd>( g_gl.clearDepthValue, 0.0, 1.0 ), ( UINT8 )( g_gl.clearStencilValue & 0xFF ), 1, &clearRect );
	}
}
...
{
	auto		Quantize = []( float v, float steps ) -> float { return floorf( v * steps + 0.5f ) / steps; };
	...
	if( bound == 0 || g_gl.textures.empty() ) {
		switch( pname ) {
			case GL_TEXTURE_MIN_FILTER:
				g_gl.defaultMinFilter = value;
				break;
			case GL_TEXTURE_MAG_FILTER:
				g_gl.defaultMagFilter = value;
				break;
			case GL_TEXTURE_WRAP_S:
				g_gl.defaultWrapS = value;
				break;
			case GL_TEXTURE_WRAP_T:
				g_gl.defaultWrapT = value;
				break;
			default:
				break;
		}
...
extern "C" void APIENTRY glTexImage2D( GLenum, GLint, GLint internalFormat, GLsizei width, GLsizei height, GLint, GLenum format, GLenum type, const GLvoid* pixels )
{
...
extern "C" void APIENTRY glTexSubImage2D( GLenum, GLint, GLint xoffset, GLint yoffset, GLsizei width, GLsizei height, GLenum format, GLenum type, const GLvoid* pixels )
{
	auto it = g_gl.textures.find( g_gl.boundTexture[g_gl.activeTextureUnit] );
...
extern "C" void glLightScene( void )
{
	...
	//
	QD3D12_TransitionResource( cl, sceneColor, g_gl.backBufferState[g_gl.frameIndex], D3D12_RESOURCE_STATE_NON_PIXEL_SHADER_RESOURCE );

	QD3D12_TransitionResource( cl, sceneNormal, g_gl.normalBufferState[g_gl.frameIndex], D3D12_RESOURCE_STATE_NON_PIXEL_SHADER_RESOURCE );

	QD3D12_TransitionResource( cl, scenePosition, g_gl.positionBufferState[g_gl.frameIndex], D3D12_RESOURCE_STATE_NON_PIXEL_SHADER_RESOURCE );

	D3D12_RESOURCE_STATES depthState = D3D12_RESOURCE_STATE_DEPTH_WRITE;
	QD3D12_TransitionResource( cl, sceneDepth, depthState, D3D12_RESOURCE_STATE_NON_PIXEL_SHADER_RESOURCE );

	QD3D12_TransitionResource( cl, lightingTex->texture.Get(), g_lightingTextureState, D3D12_RESOURCE_STATE_UNORDERED_ACCESS );
	...
	if( !glRaytracingLightingExecute( &pass ) ) {
		...
		QD3D12_TransitionResource( cl, sceneColor, g_gl.backBufferState[g_gl.frameIndex], D3D12_RESOURCE_STATE_RENDER_TARGET );

		QD3D12_TransitionResource( cl, sceneNormal, g_gl.normalBufferState[g_gl.frameIndex], D3D12_RESOURCE_STATE_RENDER_TARGET );

		QD3D12_TransitionResource( cl, scenePosition, g_gl.positionBufferState[g_gl.frameIndex], D3D12_RESOURCE_STATE_RENDER_TARGET );
	...
	//
	QD3D12_TransitionResource( cl, lightingTex->texture.Get(), g_lightingTextureState, D3D12_RESOURCE_STATE_PIXEL_SHADER_RESOURCE );
	...
	//
	D3D12_CPU_DESCRIPTOR_HANDLE restoreRtvs[3] = { CurrentRTV(), CurrentNormalRTV(), CurrentPositionRTV() };
...
{
	return g_gl.buffers.find( buffer ) != g_gl.buffers.end() ? GL_TRUE : GL_FALSE;
}
...
{
	return g_gl.queries.find( id ) != g_gl.queries.end() ? GL_TRUE : GL_FALSE;
}
