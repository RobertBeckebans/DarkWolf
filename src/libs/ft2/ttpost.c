...
static FT_Error Load_Format_20( TT_Face face, FT_Stream stream )
{
	...
	/* load the indices */
	{
		...
		if( ALLOC_ARRAY( glyph_indices, num_glyphs, FT_UShort ) || ACCESS_Frame( num_glyphs * 2L ) ) {
			goto Fail;
		}
	...
	/* now load the name strings */
	{
		...
		for( n = 0; n < num_names; n++ ) {
			...
			if( READ_Byte( len ) || ALLOC_ARRAY( name_strings[n], len + 1, FT_Char ) || FILE_Read( name_strings[n], len ) ) {
				goto Fail1;
			}
