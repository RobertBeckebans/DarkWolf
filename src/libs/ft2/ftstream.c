...
BASE_FUNC( void ) FT_New_Memory_Stream( FT_Library library, FT_Byte* base, FT_ULong size, FT_Stream stream )
{
...
BASE_FUNC( FT_Error ) FT_Read_Stream( FT_Stream stream, FT_Byte* buffer, FT_ULong count )
{
	return FT_Read_Stream_At( stream, stream->pos, buffer, count );
}

BASE_FUNC( FT_Error ) FT_Read_Stream_At( FT_Stream stream, FT_ULong pos, FT_Byte* buffer, FT_ULong count )
{
...
BASE_FUNC( FT_Error ) FT_Extract_Frame( FT_Stream stream, FT_ULong count, FT_Byte** pbytes )
{
	FT_Error error;
...
BASE_FUNC( FT_Error ) FT_Read_Fields( FT_Stream stream, const FT_Frame_Field* fields, void* structure )
{
	FT_Error error;
	...
	do {
		...
		switch( fields->value ) {
			...
			case ft_frame_ulong_le: /* read a 4-byte little-endian long */
			{
				...
				if( p + 3 < stream->limit ) {
					value = ( FT_ULong )p[0] | ( ( FT_ULong )p[1] << 8 ) | ( ( FT_ULong )p[2] << 16 ) | ( ( FT_ULong )p[3] << 24 );
					stream->cursor += 4;
				}
				sign_shift = 0;
				break;
