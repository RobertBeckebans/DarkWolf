...
FT_Error TT_CharMap_Load( TT_Face face, TT_CMapTable* cmap, FT_Stream stream )
{
	...
	switch( cmap->format ) {
		case 0:
			...
			if( ALLOC( cmap0->glyphIdArray, 256L ) || FILE_Read( cmap0->glyphIdArray, 256L ) ) {
				goto Fail;
			}
		...
		case 2:
			...
			if( ALLOC_ARRAY( cmap2->subHeaderKeys, 256, FT_UShort ) || ACCESS_Frame( 512L ) ) {
				goto Fail;
			}
			...
			if( ALLOC_ARRAY( cmap2->subHeaders, num_SH + 1, TT_CMap2SubHeader ) || ACCESS_Frame( ( num_SH + 1 ) * 8L ) ) {
				goto Fail;
			}
			...
			if( ALLOC_ARRAY( cmap2->glyphIdArray, l, FT_UShort ) || ACCESS_Frame( l * 2L ) ) {
				goto Fail;
			}
		...
		case 4:
			...
			if( ALLOC_ARRAY( cmap4->segments, num_Seg, TT_CMap4Segment ) || ACCESS_Frame( ( num_Seg * 4 + 1 ) * 2L ) ) {
				goto Fail;
			}
			...
			if( ALLOC_ARRAY( cmap4->glyphIdArray, l, FT_UShort ) || ACCESS_Frame( l * 2L ) ) {
				goto Fail;
			}
		...
		case 6:
			...
			if( ALLOC_ARRAY( cmap6->glyphIdArray, cmap6->entryCount, FT_Short ) || ACCESS_Frame( l * 2L ) ) {
				goto Fail;
			}
...
static FT_UInt code_to_index4( TT_CMapTable* cmap, FT_ULong charCode )
{
	...
	if( ( FT_ULong )( charCode - seg4->startCount ) < ( FT_ULong )( seg4->endCount - seg4->startCount ) ) {
		goto Found;
	}
...
Found:
	...
	if( seg4->idRangeOffset == 0 ) {
		...
		index1 = seg4->idRangeOffset / 2 + ( charCode - seg4->startCount ) + ( seg4 - cmap4->segments ) - segCount;
