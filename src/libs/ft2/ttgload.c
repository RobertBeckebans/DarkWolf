...
/*                                                                       */
LOCAL_FUNC
void TT_Get_Metrics( TT_HoriHeader* header, FT_UInt index, FT_Short* bearing, FT_UShort* advance )
{
...
/*                                                                       */
static void Get_HMetrics( TT_Face face, FT_UInt index, FT_Bool check, FT_Short* lsb, FT_UShort* aw )
{
...
#define cur_to_org( n, zone ) MEM_Copy( ( zone )->org, ( zone )->cur, n * sizeof( FT_Vector ) )

#define org_to_cur( n, zone ) MEM_Copy( ( zone )->cur, ( zone )->org, n * sizeof( FT_Vector ) )
...
static FT_Error TT_Access_Glyph_Frame( TT_Loader* loader, FT_UInt glyph_index, FT_ULong offset, FT_UInt byte_count )
{
	FT_Error  error;
...
static FT_Error TT_Load_Simple_Glyph( TT_Loader* load )
{
	...
	if( ( load->load_flags & ( FT_LOAD_NO_SCALE | FT_LOAD_NO_HINTING ) ) == 0 && load->instructions ) {
...
static FT_Error TT_Process_Simple_Glyph( TT_Loader* load, FT_Bool debug )
{
	...
	if( IS_HINTED( load->load_flags ) ) {
		...
		if( n_ins > 0 ) {
			error = TT_Set_CodeRange( load->exec, tt_coderange_glyph, load->exec->glyphIns, n_ins );
			if( error ) {
				goto Exit;
			}

			load->exec->is_composite	 = FALSE;
			load->exec->pedantic_hinting = ( FT_Bool )( load->load_flags & FT_LOAD_PEDANTIC );
			load->exec->pts				 = *zone;
...
static FT_Error load_truetype_glyph( TT_Loader* loader, FT_UInt glyph_index )
{
	...
	/* get horizontal metrics */
	{
		...
		Get_HMetrics( face, index, ( FT_Bool ) !( loader->load_flags & FT_LOAD_IGNORE_GLOBAL_ADVANCE_WIDTH ), &left_bearing, &advance_width );
	...
	/* otherwise, load a composite! */
		...
		/* Now, read each subglyph independently. */
		{
			...
			for( n = 0; n < ( FT_Int )num_subglyphs; n++ ) {
				...
				if( subglyph->flags & ( WE_HAVE_A_SCALE | WE_HAVE_AN_XY_SCALE | WE_HAVE_A_2X2 ) ) {
			...
			if( num_subglyphs > 0 && loader->exec && ins_pos > 0 && subglyph->flags & WE_HAVE_INSTR ) {
				FT_UShort	   n_ins;
				...
				if( IS_HINTED( loader->load_flags ) && n_ins > 0 ) {
					...
					exec->pedantic_hinting = ( FT_Bool )( loader->load_flags & FT_LOAD_PEDANTIC );
...
static void compute_glyph_metrics( TT_Loader* loader, FT_UInt glyph_index )
{
	...
	/* by the base layer.                                                 */
	{
		...
		/*                                                                */
		if( face->postscript.isFixedPitch && ( loader->load_flags & FT_LOAD_IGNORE_GLOBAL_ADVANCE_WIDTH ) == 0 ) {
...

	{
		...
		if( face->vertical_info && face->vertical.number_Of_VMetrics > 0 ) {
			...
			TT_Get_Metrics( ( TT_HoriHeader* )&face->vertical, glyph_index, &top_bearing, &advance_height );
		} else {
			...
			if( face->os2.version != 0xFFFF ) {
				...
				advance_height = ( FT_UShort )( face->os2.sTypoAscender - face->os2.sTypoDescender + face->os2.sTypoLineGap );
			} else {
				top_bearing	   = face->horizontal.Line_Gap / 2;
				advance_height = ( FT_UShort )( face->horizontal.Ascender + face->horizontal.Descender + face->horizontal.Line_Gap );
			}
		}
...
/*                                                                       */
LOCAL_FUNC
FT_Error TT_Load_Glyph( TT_Size size, TT_GlyphSlot glyph, FT_UShort glyph_index, FT_UInt load_flags )
{
	SFNT_Interface* sfnt;
	TT_Face			face;
	FT_Stream		stream;
	...
	if( size && ( load_flags & FT_LOAD_NO_BITMAP ) == 0 && sfnt->load_sbits ) {
		...
		error = sfnt->load_sbit_image( face, size->root.metrics.x_ppem, size->root.metrics.y_ppem, glyph_index, load_flags, stream, &glyph->bitmap, &metrics );
		if( !error ) {
