...
static void ah_align_serif_edge( AH_Hinter* hinter, AH_Edge* base, AH_Edge* serif )
{
	FT_Pos dist;
	FT_Pos sign = 1;
...
LOCAL_FUNC
void ah_hinter_hint_edges( AH_Hinter* hinter, int no_horz_edges, int no_vert_edges )
{
		...
		if( edges < edge_limit ) {
			for( point = points; point < point_limit; point++ ) {
				...
				/* otherwise, interpolate the point in between */
				{
					...
					u = before->pos + FT_MulDiv( fu - before->fpos, after->pos - before->pos, after->fpos - before->fpos );
				}
...
FT_Error ah_hinter_new( FT_Library library, AH_Hinter** ahinter )
{
	...
	*ahinter = 0;
	...
	error = ah_outline_new( memory, &hinter->glyph ) || ah_loader_new( memory, &hinter->loader ) || ah_loader_create_extra( hinter->loader );
	if( error ) {
		goto Exit;
	}
...
static FT_Error ah_hinter_load( AH_Hinter* hinter, FT_UInt glyph_index, FT_UInt load_flags, FT_UInt depth )
{
	...
	switch( slot->format ) {
		case ft_glyph_format_outline:
			...
			MEM_Copy( gloader->current.extra_points, slot->outline.points, slot->outline.n_points * sizeof( FT_Vector ) );

			MEM_Copy( gloader->current.outline.contours, slot->outline.contours, slot->outline.n_contours * sizeof( short ) );

			MEM_Copy( gloader->current.outline.tags, slot->outline.tags, slot->outline.n_points * sizeof( char ) );
		...
		case ft_glyph_format_composite: {
			...
			MEM_Copy( gloader->current.subglyphs, slot->subglyphs, num_subglyphs * sizeof( FT_SubGlyph ) );
			...
			for( nn = 0; nn < num_subglyphs; nn++ ) {
				...
				if( !( subglyph->flags & FT_SUBGLYPH_FLAG_ARGS_ARE_XY_VALUES ) ) {
					...
					if( start_point + k >= num_base_points || l >= ( FT_UInt )num_new_points ) {
