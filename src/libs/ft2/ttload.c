...
FT_Error TT_Load_SFNT_Header( TT_Face face, FT_Stream stream, FT_Long face_index, SFNT_Header* sfnt )
{
	...
	const FT_Frame_Field sfnt_header_fields[] = { FT_FRAME_START( 8 ),
	...
	const FT_Frame_Field ttc_header_fields[] = { FT_FRAME_START( 8 ), FT_FRAME_LONG( TTC_Header, version ), FT_FRAME_LONG( TTC_Header, count ), FT_FRAME_END };
	...
	if( format_tag == TTAG_ttcf ) {
		...
		if( ALLOC_ARRAY( face->ttc_header.offsets, face->ttc_header.count, FT_ULong ) || ACCESS_Frame( face->ttc_header.count * 4L ) ) {
			goto Exit;
		}
		...
		if( FILE_Seek( face->ttc_header.offsets[face_index] ) || READ_Long( format_tag ) ) {
			goto Exit;
		}
	}
	...
	/* now, check the values of `num_tables', `seach_range', etc. */
	{
		...
		/*                                                                  */
		if( num_tables == 0 || entry_selector > num_tables || entry_selector * 2 <= num_tables ) {
...
FT_Error TT_Load_Header( TT_Face face, FT_Stream stream )
{
	...
	static const FT_Frame_Field header_fields[] = { FT_FRAME_START( 54 ),
...
FT_Error TT_Load_MaxProfile( TT_Face face, FT_Stream stream )
{
	...
	const FT_Frame_Field maxp_fields[] = { FT_FRAME_START( 32 ),
	...
	face->root.max_points = MAX( maxProfile->maxCompositePoints, maxProfile->maxPoints );

	face->root.max_contours = MAX( maxProfile->maxCompositeContours, maxProfile->maxContours );
...
/*    FreeType error code.  0 means success.                             */
/*                                                                       */
static FT_Error TT_Load_Metrics( TT_Face face, FT_Stream stream, FT_Bool vertical )
{
	FT_Error		  error;
	FT_Memory		  memory = stream->memory;
	...
	if( num_shorts < 0 ) {
		...
		error = vertical ? TT_Err_Invalid_Vert_Metrics : TT_Err_Invalid_Horiz_Metrics;
		goto Exit;
	}

	if( ALLOC_ARRAY( *longs, num_longs, TT_LongMetrics ) || ALLOC_ARRAY( *shorts, num_shorts, TT_ShortMetrics ) ) {
		goto Exit;
	}
...
FT_Error TT_Load_Metrics_Header( TT_Face face, FT_Stream stream, FT_Bool vertical )
{
	...
	const FT_Frame_Field metrics_header_fields[] = { FT_FRAME_START( 36 ),
...
FT_Error TT_Load_Names( TT_Face face, FT_Stream stream )
{
	...
	const FT_Frame_Field name_table_fields[] = {
		FT_FRAME_START( 6 ), FT_FRAME_USHORT( TT_NameTable, format ), FT_FRAME_USHORT( TT_NameTable, numNameRecords ), FT_FRAME_USHORT( TT_NameTable, storageOffset ), FT_FRAME_END
	};

	const FT_Frame_Field name_record_fields[] = { /* no FT_FRAME_START */
	...
	if( ALLOC_ARRAY( names->names, names->numNameRecords, TT_NameRec ) || ACCESS_Frame( names->numNameRecords * 12L ) ) {
		goto Exit;
	}
	...
	if( storageSize > 0 ) {
		...
		if( ALLOC( names->storage, storageSize ) || FILE_Read_At( table_pos + names->storageOffset, names->storage, storageSize ) ) {
			goto Exit;
		}
		...
		/* Print Name Record Table in case of debugging */
		{
			...
			for( ; cur < limit; cur++ ) {
				...
				FT_TRACE3( ( "%d %d %x %d\n  ", cur->platformID, cur->encodingID, cur->languageID, cur->nameID ) );
...
FT_Error TT_Load_CMap( TT_Face face, FT_Stream stream )
{
	...
	const FT_Frame_Field cmap_fields[] = { FT_FRAME_START( 4 ), FT_FRAME_USHORT( TT_CMapDir, tableVersionNumber ), FT_FRAME_USHORT( TT_CMapDir, numCMaps ), FT_FRAME_END };

	const FT_Frame_Field cmap_rec_fields[] = {
		FT_FRAME_START( 6 ), FT_FRAME_USHORT( TT_CMapTable, format ), FT_FRAME_USHORT( TT_CMapTable, length ), FT_FRAME_USHORT( TT_CMapTable, version ), FT_FRAME_END
	};
	...
	face->num_charmaps = cmap_dir.numCMaps;
	{
		...
		for( charmap = face->charmaps; charmap < limit; charmap++ ) {
			...
			if( FILE_Seek( table_start + ( FT_Long )cmap->offset ) || READ_Fields( cmap_rec_fields, cmap ) ) {
				goto Exit;
			}
...
FT_Error TT_Load_OS2( TT_Face face, FT_Stream stream )
{
	...
	const FT_Frame_Field os2_fields[] = { FT_FRAME_START( 78 ),
	...
	const FT_Frame_Field os2_fields_extra[] = { FT_FRAME_START( 8 ), FT_FRAME_ULONG( TT_OS2, ulCodePageRange1 ), FT_FRAME_ULONG( TT_OS2, ulCodePageRange2 ), FT_FRAME_END };
...
FT_Error TT_Load_PostScript( TT_Face face, FT_Stream stream )
{
	...
	static const FT_Frame_Field post_fields[] = { FT_FRAME_START( 32 ),
...
{
	static const FT_Frame_Field pclt_fields[] = { FT_FRAME_START( 54 ),
...
FT_Error TT_Load_Gasp( TT_Face face, FT_Stream stream )
{
	...
	if( ALLOC_ARRAY( gaspranges, num_ranges, TT_GaspRange ) || ACCESS_Frame( num_ranges * 4L ) ) {
		goto Exit;
	}
...
FT_Error TT_Load_Kern( TT_Face face, FT_Stream stream )
{
	...
	for( n = 0; n < num_tables; n++ ) {
		...
		if( coverage == 0x0001 ) {
			...
			if( ALLOC_ARRAY( face->kern_pairs, num_pairs, TT_Kern_0_Pair ) || ACCESS_Frame( 6L * num_pairs ) ) {
				goto Exit;
			}
...
FT_Error TT_Load_Hdmx( TT_Face face, FT_Stream stream )
{
...

	{
		...
		for( ; cur < limit; cur++ ) {
			...
			if( ALLOC( cur->widths, num_glyphs ) || FILE_Read( cur->widths, num_glyphs ) ) {
				goto Exit;
			}
