...
static FT_Error ft_bitmap_copy( FT_Memory memory, FT_Bitmap* source, FT_Bitmap* target )
{
	FT_Error error;
...
static FT_Error ft_bitmap_glyph_copy( FT_BitmapGlyph source, FT_BitmapGlyph target )
{
...
const FT_Glyph_Class ft_bitmap_glyph_class = { sizeof( FT_BitmapGlyphRec ),
...
static FT_Error		 ft_outline_glyph_init( FT_OutlineGlyph glyph, FT_GlyphSlot slot )
{
	FT_Error	error	= FT_Err_Ok;
	FT_Library	library = FT_GLYPH( glyph )->library;
	...
	error = FT_Outline_New( library, source->n_points, source->n_contours, &glyph->outline );
	if( error ) {
		goto Exit;
	}

	/* copy it */
	MEM_Copy( target->points, source->points, source->n_points * sizeof( FT_Vector ) );

	MEM_Copy( target->tags, source->tags, source->n_points * sizeof( FT_Byte ) );

	MEM_Copy( target->contours, source->contours, source->n_contours * sizeof( FT_Short ) );
...
static FT_Error ft_outline_glyph_copy( FT_OutlineGlyph source, FT_OutlineGlyph target )
{
	FT_Error   error;
	FT_Library library = FT_GLYPH( source )->library;

	error = FT_Outline_New( library, source->outline.n_points, source->outline.n_contours, &target->outline );
	if( !error ) {
...
static void ft_outline_glyph_transform( FT_OutlineGlyph glyph, FT_Matrix* matrix, FT_Vector* delta )
{
	if( matrix ) {
...
static FT_Error ft_outline_glyph_prepare( FT_OutlineGlyph glyph, FT_GlyphSlot slot )
{
...
/*    advance vector.                                                    */
/*                                                                       */
FT_EXPORT_FUNC( FT_Error ) FT_Glyph_Transform( FT_Glyph glyph, FT_Matrix* matrix, FT_Vector* delta )
{
	const FT_Glyph_Class* clazz;
	FT_Error			  error = FT_Err_Ok;
...
/*                                                                       */
FT_EXPORT_FUNC( void ) FT_Glyph_Get_CBox( FT_Glyph glyph, FT_UInt bbox_mode, FT_BBox* cbox )
{
	const FT_Glyph_Class* clazz;
	FT_Error			  error = FT_Err_Ok;
...
/*    scalable.                                                          */
/*                                                                       */
FT_EXPORT_FUNC( FT_Error ) FT_Glyph_To_Bitmap( FT_Glyph* the_glyph, FT_ULong render_mode, FT_Vector* origin, FT_Bool destroy )
{
	...
	error = ft_new_glyph( glyph->library, &ft_bitmap_glyph_class, ( FT_Glyph* )&bitmap );
	if( error ) {
		goto Exit;
	}

	/* prepare dummy slot for rendering */
	error = clazz->glyph_prepare( glyph, &dummy ) || FT_Render_Glyph_Internal( glyph->library, &dummy, render_mode );
