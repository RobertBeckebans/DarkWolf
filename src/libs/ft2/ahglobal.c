...
static const char* blue_chars[ah_blue_max] = { "THEZOCQS", "HEZLOCUS", "xzroesc", "xzroesc", "pqgjy" };
	...
	for( blue = ah_blue_capital_top; blue < ah_blue_max; blue++ ) {
		...
		for( ; p < limit; p++ ) {
			...
			/* lies, then see its previous and next points                  */
			{
				...
				round = FT_CURVE_TAG( glyph->outline.tags[prev] ) != FT_Curve_Tag_On || FT_CURVE_TAG( glyph->outline.tags[next] ) != FT_Curve_Tag_On;
