...
static FT_String* Get_Name( TT_Face face, FT_UShort nameid )
{
	...
	for( n = 0; n < face->name_table.numNameRecords; n++, rec++ ) {
		if( rec->nameID == nameid ) {
			...
			if( rec->platformID == TT_PLATFORM_MICROSOFT && rec->encodingID <= TT_MS_ID_UNICODE_CS && ( rec->languageID & 0x3FF ) == 0x009 ) {
				found = 1;
			}
			...
			else if( rec->platformID == TT_PLATFORM_MACINTOSH && rec->languageID == TT_MAC_ID_ROMAN ) {
				found	   = 1;
				wide_chars = 0;
			}
...
LOCAL_FUNC
FT_Error SFNT_Init_Face( FT_Stream stream, TT_Face face, FT_Int face_index, FT_Int num_params, FT_Parameter* params )
{
	FT_Error		error;
	...
	if( !face->psnames ) {
		face->psnames = ( PSNames_Interface* )FT_Get_Module_Interface( library, "psnames" );
	}
...
LOCAL_FUNC
FT_Error SFNT_Load_Face( FT_Stream stream, TT_Face face, FT_Int face_index, FT_Int num_params, FT_Parameter* params )
{
	FT_Error		error;
	...
	/* now set up root fields */
	{
		...
		if( face->num_sbit_strikes ) {
			...
			if( ALLOC_ARRAY( root->available_sizes, face->num_sbit_strikes, FT_Bitmap_Size ) ) {
				return error;
			}
		...
		root->max_advance_height = face->vertical_info ? face->vertical.advance_Height_Max : root->height;
