...
typedef struct FT_Frame_Field_ {
	...
	#define FT_FRAME_FIELD( frame_op, struct_type, field ) { frame_op, sizeof( FT_FIELD_REF( struct_type, field ) ), ( FT_UShort )( char* )&FT_FIELD_REF( struct_type, field ) }
	...
	#define FT_FRAME_BYTES( struct_type, field, count )	   { ft_frame_bytes, count, ( FT_UShort )( char* )&FT_FIELD_REF( struct_type, field ) }
	...
	#define NEXT_Short( buffer )						   ( buffer += 2, ( ( short )( ( signed char )buffer[-2] << 8 ) | ( unsigned char )buffer[-1] ) )

	#define NEXT_UShort( buffer )						   ( ( unsigned short )NEXT_Short( buffer ) )

	#define NEXT_Offset( buffer )						   ( buffer += 3, ( ( ( long )( signed char )buffer[-3] << 16 ) | ( ( long )( unsigned char )buffer[-2] << 8 ) | ( long )( unsigned char )buffer[-1] ) )

	#define NEXT_UOffset( buffer )						   ( ( unsigned long )NEXT_Offset( buffer ) )

	#define NEXT_Long( buffer ) \
		( buffer += 4,          \
			( ( ( long )( signed char )buffer[-4] << 24 ) | ( ( long )( unsigned char )buffer[-3] << 16 ) | ( ( long )( unsigned char )buffer[-2] << 8 ) | ( long )( unsigned char )buffer[-1] ) )

	#define NEXT_ULong( buffer )	 ( ( unsigned long )NEXT_Long( buffer ) )

	#define NEXT_ShortLE( buffer )	 ( buffer += 2, ( ( short )( ( signed char )buffer[-1] << 8 ) | ( unsigned char )buffer[-2] ) )

	#define NEXT_UShortLE( buffer )	 ( ( unsigned short )NEXT_ShortLE( buffer ) )

	#define NEXT_OffsetLE( buffer )	 ( buffer += 3, ( ( ( long )( signed char )buffer[-1] << 16 ) | ( ( long )( unsigned char )buffer[-2] << 8 ) | ( long )( unsigned char )buffer[-3] ) )

	#define NEXT_UOffsetLE( buffer ) ( ( unsigned long )NEXT_OffsetLE( buffer ) )

	#define NEXT_LongLE( buffer ) \
		( buffer += 4,            \
			( ( ( long )( signed char )buffer[-1] << 24 ) | ( ( long )( unsigned char )buffer[-2] << 16 ) | ( ( long )( unsigned char )buffer[-3] << 8 ) | ( long )( unsigned char )buffer[-4] ) )
...
BASE_DEF( void ) FT_New_Memory_Stream( FT_Library library, FT_Byte* base, FT_ULong size, FT_Stream stream );
...
BASE_DEF( FT_Error ) FT_Read_Stream( FT_Stream stream, FT_Byte* buffer, FT_ULong count );

BASE_DEF( FT_Error ) FT_Read_Stream_At( FT_Stream stream, FT_ULong pos, FT_Byte* buffer, FT_ULong count );

BASE_DEF( FT_Error ) FT_Access_Frame( FT_Stream stream, FT_ULong count );

BASE_DEF( void ) FT_Forget_Frame( FT_Stream stream );

BASE_DEF( FT_Error ) FT_Extract_Frame( FT_Stream stream, FT_ULong count, FT_Byte** pbytes );
...
BASE_DEF( FT_Error ) FT_Read_Fields( FT_Stream stream, const FT_Frame_Field* fields, void* structure );

	#define USE_Stream( resource, stream )			FT_SET_ERROR( FT_Open_Stream( resource, stream ) )
	...
	#define EXTRACT_Frame( size, bytes )			FT_SET_ERROR( FT_Extract_Frame( stream, size, ( FT_Byte** )&( bytes ) ) )
	...
	#define FILE_Read( buffer, count )				FT_SET_ERROR( FT_Read_Stream( stream, ( FT_Byte* )buffer, count ) )

	#define FILE_Read_At( position, buffer, count ) FT_SET_ERROR( FT_Read_Stream_At( stream, position, ( FT_Byte* )buffer, count ) )

	#define READ_Fields( fields, object )			( ( error = FT_Read_Fields( stream, fields, object ) ) != FT_Err_Ok )
